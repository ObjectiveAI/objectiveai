...
///
/// pgwire as the plugin wrote it, on its way to the caller's database.
///
/// Opaque, for the reason the whole conduit is: it is never parsed, so
/// TLS negotiation and every protocol extension cross untouched. And a
/// stream rather than a message — a Postgres message larger than one
/// frame simply spans several, and both ends reassemble, as they would
/// from a socket.
///
/// # The mirror of the caller's, and deliberately its own type
///
/// The [`client's`](crate::endpoints::mcp_plugin::run::client::channel_response::postgres::Frame)
...
///
/// The plugin's socket ended, and no further byte will ever be written
/// by this connection. That is the signal that has no other way to
/// travel: a plugin that crashes sends no pgwire `Terminate`, so
/// without it a caller would hold a backend for a client that no
/// longer exists.
///
/// It is the whole reason this channel is the caller's to open. A
/// provider cannot finish a channel it is asking on, so the writes had
/// to become an answer for the provider to be able to end them.
