...
///
/// Only a finish ends it. What a provider does with a write that ended in
/// an error is not specified here.
///
/// # The failure is this endpoint's, the bytes are not
///
/// [`Body`](Self::Body) carries
/// [`write_bytes::response::Frame`](crate::shared::container::write_bytes::response::Frame),
/// which is what a piece of a file is anywhere. The
/// [`Error`](Self::Error) beside it is not shared, because what can go
/// wrong belongs to the exchange rather than to the content — a
/// vocabulary of failures defined once in `shared` would be one
/// endpoint's needs imposed on every endpoint that ever streams a
/// write.
...
pub enum Frame<'a> {
    ...
    /// A failure. Tag `1`.
    ///
    /// The connector cannot supply the content it was asked for — the
