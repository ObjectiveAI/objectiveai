...
use crate::shared::container::transfer;
use crate::shared::error::Error;

/// The file landed, or it did not.
///
/// A payload leads with one byte saying which — `0` for
/// [`Transferred`](Self::Transferred), `1` for [`Error`](Self::Error) — and the rest
...
/// specified here.
///
/// # The failure is this endpoint's, the answer is not
///
/// [`Transferred`](Self::Transferred) carries
/// [`transfer::response::Frame`](crate::shared::container::transfer::response::Frame),
/// which means the same thing wherever the exchange happens. The
/// [`Error`](Self::Error) beside it is not shared, because what can go
/// wrong belongs to the exchange rather than to what it carries — a
/// vocabulary of failures defined once in `shared` would be one
/// endpoint's needs imposed on every endpoint that ever moves a file between containers.
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => {
                f.write_str("transfer response frame is empty")
            }
