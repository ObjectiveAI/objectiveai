...
/// The answer to a resource read, or the reason there is not one.
///
/// The payload of a
/// [`ClientFrame::ChannelResponse`](crate::frame::client::ClientFrame::ChannelResponse)
/// on a channel opened by
/// [`channel_request::Frame::McpReadResource`](crate::endpoints::agentic_loop::run::server::channel_request::Frame::McpReadResource).
///
...
/// An answer to a resource read that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => {
                f.write_str("read_resource response frame is empty")
            }
            FrameError::UnknownTag(tag) => {
                write!(f, "unknown read_resource response frame tag {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "read_resource response did not parse: {error}")
            }
        }
    }
}
