...
/// The answer to a resource listing, or the reason there is not one.
///
/// The payload of a
/// [`ClientFrame::ChannelResponse`](crate::frame::client::ClientFrame::ChannelResponse)
/// on a channel opened by
/// [`channel_request::Frame::McpListResources`](crate::endpoints::agentic_loop::run::server::channel_request::Frame::McpListResources).
///
...
pub enum Frame {
    /// The resources a server offers, and a cursor if there are more. Tag `0`.
...
/// An answer to a resource listing that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => {
                f.write_str("list_resources response frame is empty")
            }
            FrameError::UnknownTag(tag) => {
                write!(f, "unknown list_resources response frame tag {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "list_resources response did not parse: {error}")
            }
        }
    }
}
