...
/// Push `new` onto `chunks`, coalescing the streamable text kinds.
///
/// A stream delivers reasoning, text and refusals in fragments; a
/// record of the stream does not have to keep the fragment
/// boundaries. Each incoming chunk that is one of those three kinds,
/// arriving directly behind a chunk of the SAME kind, is pushed onto
/// that chunk instead of appended beside it — so consecutive
/// fragments become one chunk, and everything else appends as it
/// came.
///
