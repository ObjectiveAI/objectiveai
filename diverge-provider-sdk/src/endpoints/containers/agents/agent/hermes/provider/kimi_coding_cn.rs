...
///
/// APPLICATION: the harness sets `KIMI_CN_API_KEY` to
/// [`api_key`](Self::api_key) in the gateway's process environment
/// before Hermes starts.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct Provider {
    /// The discriminator. Always `kimi-coding-cn`.
    pub provider: KimiCodingCn,
    /// The API key, applied as `KIMI_CN_API_KEY`.
    pub api_key: String,
}
