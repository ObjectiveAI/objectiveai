//! What a client's request frame carries for an agent container connection.

use crate::decode::Decode;
use crate::encode::{Encode, Writer};
use crate::shared::containers::request::Connect;

/// Ask to join an agent container somebody else is running.
///
...
/// An agent container connection request that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => {
                f.write_str("agents connect request frame is empty")
            }
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected agents connect request tag {TAG}, found {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "agents connect request did not parse: {error}")
            }
        }
    }
}
