...
///    dialled. A failure is the run's `Error`, then the finish, and
///    nothing the caller may have opened meanwhile is read — it is
///    dropped with the scope, unanswered, which is what a caller
///    reads as the run never having been.
/// 2. For an agent container, the agent registered — `agent` is
///    `Some` — with the proxy's `/agent/register`; a refusal is the
///    container's own `Error`, and the container is stopped.
    ...
    G::Error: Into<Error>,
{
    ...
    directory.insert(id.clone(), Arc::clone(&scope), prepared.container.address().to_string());
    send(&scope, R::id(&Id { id: id.clone() })).await;

    let run = Arc::new(Run::new(Arc::clone(&scope), prepared.client));
