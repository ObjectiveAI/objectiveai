...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/tool/call-tool: {error}"),
            ExecuteError::Encode(error) => write!(f, "/tool/call-tool params did not serialize: {error}"),
            ExecuteError::Frame(error) => write!(f, "{error}"),
            ExecuteError::Unserved => f.write_str("the proxy could not serve /tool/call-tool"),
            ExecuteError::Text => f.write_str("/tool/call-tool carried a text message"),
            ExecuteError::Socket(error) => write!(f, "/tool/call-tool failed: {error}"),
            ExecuteError::Closed => f.write_str("/tool/call-tool ended without a close"),
        }
    }
}
