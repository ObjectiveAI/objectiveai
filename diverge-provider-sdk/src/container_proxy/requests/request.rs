...
/// | `4` | [`Vault`](Self::Vault) | the operation, as [`vault::Request`] encodes | `/vault/{channel}` |
/// | `5` | [`Command`](Self::Command) | the command, opaque | `/command/{channel}` |
/// | `6` | [`Postgres`](Self::Postgres) | none | `/postgres/{channel}` |
///
/// What each answer path carries is its module's to say: [`mcp`],
/// [`vault`], [`command`](crate::container_proxy::command),
/// [`postgres`](crate::container_proxy::postgres). Notifications are
/// not here — nothing is asked for them; the server pushes them on
/// `/mcp/notifications`.
...
impl Encode for Request<'_> {
    ...
    /// copied and an announcement is nothing.
