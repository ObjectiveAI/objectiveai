...
impl<E: fmt::Display> fmt::Display for ExecuteError<E> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/write: {error}"),
            ...
            ExecuteError::Text => f.write_str("/write carried a text message"),
            ExecuteError::Socket(error) => write!(f, "/write failed: {error}"),
            ExecuteError::Closed => f.write_str("/write ended without a close"),
        }
    }
}
