...
/// held — see [the module](super) for whose the lock is, how the TTL
...
/// ```
///
/// The TTL leads so the key can be the rest. Kind `8` on
/// `/requests`, answered on `/vault/lock/{channel}` with one
/// [`Done`](super::Done).
...
pub struct Lock<'a> {
    ...
    /// [`Unlock`](super::Unlock). `0` is refused.
    pub ttl: u32,
}
