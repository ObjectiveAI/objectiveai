...
/// Open `/run-loop`, hand over the request, and read the loop.
///
...
/// error, with the container's reason; a loop already in progress is
/// the proxy's `409`, arriving as [`Open`](ExecuteError::Open).
pub async fn execute(
    client: &ContainerClient,
    request: &request::Request,
) -> Result<ExecuteStream, ExecuteError> {
    let mut socket = client.open("/run-loop").await.map_err(ExecuteError::Open)?;
