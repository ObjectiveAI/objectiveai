//! What can go wrong answering a tool listing.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/mcp/list-tools: {error}"),
            ExecuteError::Encode(error) => {
                write!(f, "answer did not serialize: {error}")
            }
            ExecuteError::Socket(error) => write!(f, "/mcp/list-tools failed: {error}"),
        }
    }
}
