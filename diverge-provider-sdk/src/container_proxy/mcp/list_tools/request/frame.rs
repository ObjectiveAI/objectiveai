//! The frame the container sends on `/mcp/list-tools`.
...
pub struct Frame {
    ...
    /// by the container.
    pub channel: u8,
    /// What tools are there. [`None`] asks for the first page, which is
/// also what a caller with nothing to say sends — rmcp makes the
/// params optional and this keeps that, because params absent and a
/// cursor absent are different things to a server that reads them.
    pub request: mcp::list_tools::request::Request,
}
...
impl Frame {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, FrameError> {
        ...
        Ok(Frame {
            channel: *channel,
            request: mcp::list_tools::request::Request::decode(request)
