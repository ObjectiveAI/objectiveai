//! The frame the container sends on `/mcp/read-resource`.
...
pub struct Frame {
    ...
    /// by the container.
    pub channel: u8,
    /// Read one resource, by URI. The URI is the server's to interpret;
/// a relay that resolved one would be deciding what a resource is.
    pub request: mcp::read_resource::request::Request,
}
...
impl Frame {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, FrameError> {
        ...
        Ok(Frame {
            channel: *channel,
            request: mcp::read_resource::request::Request::decode(request)
