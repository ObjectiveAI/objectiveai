//! Why a frame on the `/mcp/read-resource` path could not be decoded.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Truncated => {
                f.write_str("read-resource frame is shorter than its header")
            }
            FrameError::UnknownType(byte) => {
                write!(f, "unknown read-resource frame type {byte}")
            }
            FrameError::Request(error) => {
                write!(f, "read-resource request did not parse: {error}")
            }
            FrameError::Response(error) => {
                write!(f, "read-resource response did not decode: {error}")
            }
        }
    }
}
