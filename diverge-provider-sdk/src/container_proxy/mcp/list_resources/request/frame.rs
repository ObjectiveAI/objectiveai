//! The frame the container sends on `/mcp/list-resources`.
...
pub struct Frame {
    ...
    /// by the container.
    pub channel: u8,
    /// What resources are there. The same shape as list-tools, for the
/// same reason: it is the same MCP request against a different noun.
    pub request: mcp::list_resources::request::Request,
}
...
impl Frame {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, FrameError> {
        ...
        Ok(Frame {
            channel: *channel,
            request: mcp::list_resources::request::Request::decode(request)
