...
/// Answer the ask on `channel`: open `/mcp/list-resources/{channel}`, send `response` if
/// there is one, close cleanly.
///
...
pub async fn execute(
    ...
    channel: u32,
    response: Option<&response::Frame>,
) -> Result<(), ExecuteError> {
    let mut socket = client
        .open(&format!("/mcp/list-resources/{channel}"))
        .await
        .map_err(ExecuteError::Open)?;
    if let Some(response) = response {
        let bytes = container_client::encoded(response).map_err(ExecuteError::Encode)?;
        socket
