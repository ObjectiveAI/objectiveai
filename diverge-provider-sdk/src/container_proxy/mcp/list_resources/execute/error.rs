//! What can go wrong answering a resource listing.
...
pub enum ExecuteError {
    ...
    Open(OpenError),
    /// The answer would not serialize.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/mcp/list-resources: {error}"),
            ExecuteError::Encode(error) => {
                write!(f, "answer did not serialize: {error}")
            }
            ExecuteError::Socket(error) => write!(f, "/mcp/list-resources failed: {error}"),
        }
    }
}
