//! Why a frame on the `/mcp/list-resources` path could not be decoded.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Truncated => {
                f.write_str("list-resources frame is shorter than its header")
            }
            FrameError::UnknownType(byte) => {
                write!(f, "unknown list-resources frame type {byte}")
            }
            FrameError::Request(error) => {
                write!(f, "list-resources request did not parse: {error}")
            }
            FrameError::Response(error) => {
                write!(f, "list-resources response did not decode: {error}")
            }
        }
    }
}
