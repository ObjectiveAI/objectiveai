//! Why a frame on the `/mcp/call-tool` path could not be decoded.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Truncated => {
                f.write_str("call-tool frame is shorter than its header")
            }
            FrameError::UnknownType(byte) => {
                write!(f, "unknown call-tool frame type {byte}")
            }
            FrameError::Request(error) => {
                write!(f, "call-tool request did not parse: {error}")
            }
            FrameError::Response(error) => {
                write!(f, "call-tool response did not decode: {error}")
            }
        }
    }
}
