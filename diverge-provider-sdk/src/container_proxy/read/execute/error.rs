...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/read: {error}"),
            ExecuteError::Encode(error) => {
                write!(f, "read request did not serialize: {error}")
            }
            ExecuteError::Socket(error) => write!(f, "/read failed: {error}"),
        }
    }
}
...
impl fmt::Display for ExecuteStreamError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecuteStreamError::Text => f.write_str("/read carried a text message"),
            ExecuteStreamError::Socket(error) => write!(f, "/read failed: {error}"),
            ExecuteStreamError::Closed => f.write_str("/read ended without a close"),
        }
    }
}
