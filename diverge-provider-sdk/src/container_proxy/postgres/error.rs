//! Why a frame on the `/postgres` path could not be decoded.
...
/// finish. Both directions share it.
...
pub enum FrameError {
    ...
    Truncated,
    /// A `type` this path does not define, in the direction it was
    /// read.
    ///
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Truncated => {
                f.write_str("postgres frame is shorter than its header")
            }
            FrameError::UnknownType(byte) => {
                write!(f, "unknown postgres frame type {byte}")
            }
        }
    }
}
