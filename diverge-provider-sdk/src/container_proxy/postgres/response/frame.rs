//! Frames the server sends on `/postgres`.
...
/// path — so there is no `open` in this direction.
...
pub enum Frame<'a> {
    ...
    /// caller declined to serve it at all, which looks the same from
    /// here. The proxy acts on it by shutting the driver's socket:
    /// the driver sees a server that hung up on it, which is the
    /// truth.
    Close {
