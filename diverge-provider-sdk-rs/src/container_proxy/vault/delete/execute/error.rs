//! What can go wrong answering a vault removal.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/vault/delete: {error}"),
            ExecuteError::Socket(error) => write!(f, "/vault/delete failed: {error}"),
        }
    }
}
