//! What can go wrong answering a vault unlock.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/vault/unlock: {error}"),
            ExecuteError::Socket(error) => write!(f, "/vault/unlock failed: {error}"),
        }
    }
}
