//! What can go wrong answering a resource read.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/mcp/read-resource: {error}"),
            ExecuteError::Encode(error) => {
                write!(f, "answer did not serialize: {error}")
            }
            ExecuteError::Socket(error) => write!(f, "/mcp/read-resource failed: {error}"),
        }
    }
}
