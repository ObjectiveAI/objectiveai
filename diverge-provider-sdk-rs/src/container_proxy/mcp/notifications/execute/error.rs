//! What can go wrong answering a notifications stream.
...
impl fmt::Display for ExecuteError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Open(error) => write!(f, "/mcp/notifications: {error}"),
        }
    }
}
...
impl fmt::Display for HandleError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            HandleError::Socket(error) => write!(f, "/mcp/notifications failed: {error}"),
        }
    }
}
