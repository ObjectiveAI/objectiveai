...
//!
//! The image runs an agent, and the caller talks to it. The agent — a
//! JSON value the image defines — is on the
//! [`request`](run::client::request::Frame) that makes the container,
//! fixed for its life; what the agent says rides the scope's own main
//! stream, chunk by chunk, after the id — see
//! [`run::server::response`]. The caller speaks to it with
