//! What a server's response frame carries for a tree.
...
/// |---------------------|-------|
/// | frames, then a finish | the watch ended — the server said stop, or the proxy is ending |
...
/// [`shared::filetree`](crate::shared::filetree) for when. What is an
/// error is only the watch failing to exist: a watcher not made, a
/// root that cannot be watched, a walk that died. Everything short of
/// that — a corner the proxy could not watch — is reported in the
/// tree.
...
pub enum Frame {
    ...
    /// — which is the same thing the provider answers the caller with
    /// on its own filetree channel, one hop on.
...
/// A tree response that could not be written.
#[derive(Debug)]
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FrameEncodeError::Error(error) => {
                write!(f, "tree error did not serialize: {error}")
            }
        }
    }
}
...
/// A tree response frame that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => f.write_str("tree response frame is empty"),
            FrameError::UnknownTag(tag) => {
                write!(f, "unknown tree response frame tag {tag}")
            }
            FrameError::Filetree(error) => {
                write!(f, "filetree frame did not decode: {error}")
            }
            FrameError::Error(error) => {
                write!(f, "tree error did not parse: {error}")
            }
        }
    }
}
