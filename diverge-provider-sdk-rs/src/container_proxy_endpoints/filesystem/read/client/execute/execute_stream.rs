...
impl ExecuteStream {
    ...
    fn end(&mut self, error: ExecuteStreamError) -> Poll<Option<Result<Bytes, ExecuteStreamError>>> {
        self.receiver = None;
...
impl Stream for ExecuteStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match response::Frame::decode(payload) {
            Ok(response::Frame::Body(body)) => Poll::Ready(Some(Ok(bytes.slice_ref(body.0)))),
...
pub enum ExecuteStreamError {
    ...
    /// The file was not read, or not all of it; the proxy's own words.
    Refused(Error),
}
