...
pub async fn execute(global: &GlobalContext, scoped: &ScopedContext, request: Request) -> Result<ItemStream, Error> {
    let stream: ItemStream = match request {
        Request::Enqueue(req) => {
            let value = enqueue::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Enqueue(value)))
        }
        Request::EnqueueRequestSchema(req) => {
            let value = enqueue::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::EnqueueRequestSchema(value)))
        }
        Request::EnqueueResponseSchema(req) => {
            let value = enqueue::response_schema::execute(global, scoped, req).await?;
        ...
        Request::Instances(req) => {
            let inner = instances::execute(global, scoped, req).await?;
        ...
        Request::Message(req) => {
            let value = message::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Message(value)))
        }
        Request::MessageRequestSchema(req) => {
            let value = message::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::MessageRequestSchema(value)))
        }
        Request::MessageResponseSchema(req) => {
            let value = message::response_schema::execute(global, scoped, req).await?;
        ...
        Request::Queue(req) => {
            let inner = queue::execute(global, scoped, req).await?;
        ...
        Request::Tags(req) => {
            let inner = tags::execute(global, scoped, req).await?;
            Box::pin(inner.map(|r| r.map(ResponseItem::Tags)))
        }
        Request::Wait(req) => {
            let value = wait::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Wait(value)))
        }
        Request::WaitRequestSchema(req) => {
            let value = wait::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::WaitRequestSchema(value)))
        }
        Request::WaitResponseSchema(req) => {
            let value = wait::response_schema::execute(global, scoped, req).await?;
