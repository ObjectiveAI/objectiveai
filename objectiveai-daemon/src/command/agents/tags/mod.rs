...
pub async fn execute(global: &GlobalContext, scoped: &ScopedContext, request: Request) -> Result<ItemStream, Error> {
    let stream: ItemStream = match request {
        Request::Lookup(req) => {
            let value = lookup::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Lookup(value)))
        }
        Request::LookupRequestSchema(req) => {
            let value = lookup::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::LookupRequestSchema(value)))
        }
        Request::LookupResponseSchema(req) => {
            let value = lookup::response_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::LookupResponseSchema(value)))
        }
        Request::Apply(req) => {
            let value = apply::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Apply(value)))
        }
        Request::ApplyRequestSchema(req) => {
            let value = apply::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::ApplyRequestSchema(value)))
        }
        Request::ApplyResponseSchema(req) => {
            let value = apply::response_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::ApplyResponseSchema(value)))
        }
        Request::Remove(req) => {
            let value = remove::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::Remove(value)))
        }
        Request::RemoveRequestSchema(req) => {
            let value = remove::request_schema::execute(global, scoped, req).await?;
            once(Ok(ResponseItem::RemoveRequestSchema(value)))
        }
        Request::RemoveResponseSchema(req) => {
            let value = remove::response_schema::execute(global, scoped, req).await?;
