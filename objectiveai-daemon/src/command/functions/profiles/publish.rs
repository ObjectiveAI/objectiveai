//! `functions profiles publish` — write a `RemoteProfile` to a
//! repository on the filesystem and return its content sha.

use objectiveai_sdk::cli::command::functions::profiles::publish::{
    Request, RequestBody, RequestPublishMessage, Response,
};
use objectiveai_sdk::functions::RemoteProfile;
...
async fn resolve_body(global: &GlobalContext, scoped: &ScopedContext, body: RequestBody) -> Result<RemoteProfile, Error> {
    match body {
