...
//! ([`crate::command::laboratories::spawn::converge`] — ack-gated, so
//! the command returns once the live dial list matches disk). No live
//! host ⇒ the converge no-ops immediately (never spawns one); the
//! next `laboratories spawn` converges from config.
