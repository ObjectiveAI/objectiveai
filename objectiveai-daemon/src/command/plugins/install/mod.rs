...
use futures::Stream;
use objectiveai_sdk::cli::command::plugins::install::{Request, Response};
...
pub async fn execute(global: &GlobalContext, scoped: &ScopedContext, request: Request) -> Result<ItemStream, Error> {
    let stream: ItemStream = match request {
        Request::Filesystem(req) => {
            let value = filesystem::execute(global, scoped, req).await?;
            once(Ok(Response::Filesystem(value)))
        }
        Request::FilesystemRequestSchema(req) => {
            let value = filesystem::request_schema::execute(global, scoped, req).await?;
            once(Ok(Response::FilesystemRequestSchema(value)))
        }
        Request::FilesystemResponseSchema(req) => {
            let value = filesystem::response_schema::execute(global, scoped, req).await?;
            once(Ok(Response::FilesystemResponseSchema(value)))
        }
        Request::Github(req) => {
            let value = github::execute(global, scoped, req).await?;
            once(Ok(Response::Github(value)))
        }
        Request::GithubRequestSchema(req) => {
            let value = github::request_schema::execute(global, scoped, req).await?;
            once(Ok(Response::GithubRequestSchema(value)))
        }
        Request::GithubResponseSchema(req) => {
            let value = github::response_schema::execute(global, scoped, req).await?;
