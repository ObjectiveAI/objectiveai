use jaq_core::{Compiler, Ctx, FilterT, RcIter, load};
use jaq_json::Val;
use serde::Serialize;
...
pub fn run_jq(
    ...
    let input_value =
        serde_json::to_value(value).map_err(super::Error::Serialize)?;
    ...
    for item in out {
        match item {
            ...
            Err(err) => {
                return Err(super::Error::JqRuntime(format!("{err:?}")));
            }
        }
    }
    Ok(results)
