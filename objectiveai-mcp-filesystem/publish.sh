#!/usr/bin/env bash
# Publishes objectiveai-mcp-filesystem to crates.io as `objectiveai-mcp-filesystem`.
#
# Usage:
#   bash objectiveai-mcp-filesystem/publish.sh                # crates.io (via GHA)
#   bash objectiveai-mcp-filesystem/publish.sh --build-only   # local cargo publish --dry-run
#
# `--test` is not supported (crates.io has no test registry).
#
# Output is captured to .logs/publish/objectiveai-mcp-filesystem.txt.
#
# Setup (one-time):
...
CRATE="objectiveai-mcp-filesystem"
