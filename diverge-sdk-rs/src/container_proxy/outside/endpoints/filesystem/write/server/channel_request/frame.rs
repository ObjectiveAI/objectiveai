//! What a server's channel request frame carries for a write.
...
/// Send the content.
///
/// What the proxy asks the server for, and the only thing: a write
...
/// the proxy opened. Opened once the request is read, before anything
/// is written.
///
...
/// The provider protocol's counterpart quotes a write id, because
/// there the content channel and the write share a scope with every
/// other write; here the scope IS the write, so a payload is nothing
/// at all.
