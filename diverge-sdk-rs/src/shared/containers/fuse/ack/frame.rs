...
pub enum Frame<'a> {
    ...
    Error(&'a str),
    /// Kind `2`. It did not, because the volume behind the mount is
    /// read only: its mode is
    /// [`ReadOnly`](crate::provider::endpoints::volumes::Mode::ReadOnly), and
    /// no change is made to it. Nothing after the kind. The program
    /// sees a read-only filesystem, `EROFS`, and every immutable ask
    /// on the same mount goes through.
    ReadOnly,
}
...
impl Encode for Frame<'_> {
    ...
    fn encode(&self, out: &mut Writer<'_>) -> Result<(), Infallible> {
        match self {
            ...
            Frame::ReadOnly => out.extend_from_slice(&[2]),
        }
        Ok(())
    }
}
