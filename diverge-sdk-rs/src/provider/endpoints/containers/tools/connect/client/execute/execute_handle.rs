//! A connection to a tool container, and everything a connector can
//! do with it.
...
use crate::provider::endpoints::containers::client::{ChannelStream, Decoded, OpenError, Scoped, UnaryError, WaitError};
...
/// The scope a connection opened, held for its life.
///
/// Every channel a connector may open into a tool container is a
/// method here: the tree watched, a file read or written, the five
/// MCP exchanges, and the disconnect — which takes nothing with it,
/// since a connector joined something it does not own. Clones share
/// the scope, and [`wait`](Self::wait) on any of them reports the
/// same end.
///
/// # Dropping it does nothing
///
/// No destructor sends a frame. Leaving is
/// [`disconnect`](Self::disconnect), or the connection going.
#[derive(Debug, Clone)]
...
impl ExecuteHandle {
    ...
    /// The connection's end: `Ok` when the scope finishes as it should
    /// — the connector left, or the container did — and the
    /// provider's error when it never was: the container was not
    /// there, or its runner said no. Resolves once and answers the
    ...
    /// Leave the container, which goes on without this connector.
