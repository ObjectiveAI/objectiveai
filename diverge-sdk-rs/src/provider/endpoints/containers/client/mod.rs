...
//! error types — is what the executors under
//! [`outside`](crate::container_proxy::outside) use as the caller's
//! do: the provider's server is the client on that wire. The asks,
//! the answerers' dispatch, the family encoders, the running scope
//! and its pending writes are the caller's alone.
//!
