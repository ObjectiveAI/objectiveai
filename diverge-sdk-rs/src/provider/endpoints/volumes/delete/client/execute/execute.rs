...
use crate::provider::endpoints::volumes::delete::server::response;
use crate::wire::frame;
use crate::shared::error::Error;

/// Destroy a volume, and wait for the answer.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecuteError::Request(error) => {
                write!(f, "volume deletion request did not serialize: {error}")
            }
            ExecuteError::Closed => f.write_str(
                "connection ended before the volume deletion answered",
            ),
            ExecuteError::Frame(error) => {
                write!(f, "volume deletion answer did not decode: {error}")
            }
            ExecuteError::Unanswered => {
                f.write_str("volume deletion finished without an answer")
            }
