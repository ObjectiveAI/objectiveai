...
use crate::provider::endpoints::volumes::edit_capacity::server::response;
...
///
/// A number is an ANSWER, and `0` is one: the volume cannot grow right now. An [`ExecuteError::Provider`] is the absence of an
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecuteError::Request(error) => {
                write!(f, "volume edit-capacity request did not serialize: {error}")
            }
            ExecuteError::Closed => f.write_str(
                "connection ended before the volume edit-capacity question answered",
            ),
            ExecuteError::Frame(error) => {
                write!(f, "volume edit-capacity answer did not decode: {error}")
            }
            ExecuteError::Unanswered => {
                f.write_str("volume edit-capacity question finished without an answer")
            }
            ExecuteError::Response(error) => {
                write!(f, "volume edit-capacity answer did not parse: {error}")
            }
            ExecuteError::Provider(_) => {
                f.write_str("the provider could not answer the volume edit-capacity question")
            }
        }
    }
}
