//! What a server's response frame carries for a volume edit-capacity question.
...
///
/// The number is how many bytes, as of this answer, the named
/// volume's size could be increased by — headroom, not a size.
...
///
/// `0` is a legal number: the volume cannot grow right now. It is an answer, not an [`Error`](Self::Error), which
...
/// [`volumes`](crate::provider::endpoints::volumes). The error is JSON, and has
...
/// A volume edit-capacity answer that could not be written.
#[derive(Debug)]
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameEncodeError::Capacity(error) => {
                write!(f, "volume edit-capacity did not serialize: {error}")
            }
            FrameEncodeError::Error(error) => {
                write!(f, "volume edit-capacity error did not serialize: {error}")
            }
        }
    }
}
...
/// A volume edit-capacity answer that could not be read.
#[derive(Debug)]
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameDecodeError::Empty => {
                f.write_str("volume edit-capacity response frame is empty")
            }
            FrameDecodeError::UnknownTag(tag) => {
                write!(f, "unknown volume edit-capacity response frame tag {tag}")
            }
            FrameDecodeError::Capacity(error) => {
                write!(f, "volume edit-capacity did not parse: {error}")
            }
            FrameDecodeError::Error(error) => {
                write!(f, "volume edit-capacity error did not parse: {error}")
            }
        }
    }
}
