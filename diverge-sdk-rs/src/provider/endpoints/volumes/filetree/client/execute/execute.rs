...
use crate::provider::endpoints::volumes::filetree::server::response;
use crate::wire::frame;
use crate::shared::error::Error;
use crate::shared::filetree::response::Node;

/// See what a volume holds, and wait for the answer.
///
...
/// wanted. The tree is the entries of the subtree the request named,
/// as a [`Snapshot`](crate::shared::filetree::response::Frame::Snapshot)
/// carries them, and nothing follows it.
///
/// # An answer and an error are two different things
///
/// The tree is an ANSWER, and it is the only one. Everything else is
/// an [`ExecuteError::Provider`] — a volume the caller cannot see, one
/// that is mounted, a path that names no directory.
///
...
pub async fn execute(
    ...
    match response::Frame::decode(payload).map_err(ExecuteError::Response)? {
        response::Frame::Tree(tree) => Ok(tree),
        response::Frame::Error(error) => Err(ExecuteError::Provider(error)),
    }
}

/// A filetree that did not produce an answer.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecuteError::Request(error) => {
                write!(f, "volume filetree request did not serialize: {error}")
            }
            ExecuteError::Closed => {
                f.write_str("connection ended before the volume filetree answered")
            }
            ExecuteError::Frame(error) => {
                write!(f, "volume filetree answer did not decode: {error}")
            }
            ExecuteError::Unanswered => {
                f.write_str("volume filetree finished without an answer")
            }
            ExecuteError::Response(error) => {
                write!(f, "volume filetree answer did not parse: {error}")
            }
            ExecuteError::Provider(_) => {
                f.write_str("the provider could not complete the volume filetree")
            }
        }
    }
}
