...
/// A read that could not be opened. Everything after the opening is
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ExecuteError::Send(error) => {
                write!(f, "volume read request could not be sent: {error}")
            }
            ExecuteError::Request(error) => {
                write!(f, "volume read request did not serialize: {error}")
            }
        }
    }
}
