...
use crate::provider::endpoints::volumes::stat::server::response;
use crate::wire::frame;
use crate::shared::error::Error;

/// Examine a volume, and wait for the answer.
///
...
///
/// A [`Stat`](response::Stat) is an ANSWER, and it is the only one.
/// Everything else is an [`ExecuteError::Provider`] — a volume the
/// caller cannot see, or one the provider could not walk.
///
...
pub async fn execute(
...
) -> Result<response::Stat, ExecuteError> {
    ...
    match response::Frame::decode(payload).map_err(ExecuteError::Response)? {
        response::Frame::Stat(stat) => Ok(stat),
        response::Frame::Error(error) => Err(ExecuteError::Provider(error)),
    }
}

/// A stat that did not produce an answer.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecuteError::Request(error) => {
                write!(f, "volume stat request did not serialize: {error}")
            }
            ExecuteError::Closed => {
                f.write_str("connection ended before the volume stat answered")
            }
            ExecuteError::Frame(error) => {
                write!(f, "volume stat answer did not decode: {error}")
            }
            ExecuteError::Unanswered => {
                f.write_str("volume stat finished without an answer")
            }
            ExecuteError::Response(error) => {
                write!(f, "volume stat answer did not parse: {error}")
            }
            ExecuteError::Provider(_) => {
                f.write_str("the provider could not complete the volume stat")
            }
        }
    }
}
