...
/// [`mode`](Self::mode) is what becomes of a change to it. That is
/// the whole of what a listing says about one, and
...
/// [`stat`](crate::provider::endpoints::volumes::stat) reports, because each
...
/// [`stat`](crate::provider::endpoints::volumes::stat) names it, a
/// [`delete`](crate::provider::endpoints::volumes::delete) names it, and a
...
pub struct Volume {
    ...
    /// [`create`](crate::provider::endpoints::volumes::create) made, the caller
    /// did. Nothing here says which, and nothing should.
    ///
    /// It is also the HANDLE, and the only one. A
    /// [`stat`](crate::provider::endpoints::volumes::stat) names a volume by
    ...
    ///
    /// For a volume a
    /// [`create`](crate::provider::endpoints::volumes::create::client::request::Frame::bytes)
    ...
    pub created: u64,
    /// The mode the volume is in: persistent, ephemeral, or read
    /// only. See [`Mode`].
    ///
    /// What a
    /// [`create`](crate::provider::endpoints::volumes::create::client::request::Frame::mode)
    /// stated, or the last
    /// [`edit`](crate::provider::endpoints::volumes::edit::client::request::Change::Mode)
    /// of it; for a volume the provider offers on its own, the
    /// provider's word. One answer for every container that mounts
    /// the volume and every serve of it — neither chooses.
    ///
    /// One byte in postcard, `0`, `1` or `2`, after `created`.
    pub mode: Mode,
}
