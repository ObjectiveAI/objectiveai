    ...
    pub async fn handle(self, cli_config: &crate::Config) -> Result<(), crate::error::Error> {
        ...
        match self {
            ...
            Commands::Subscribe { id, message_index, media_index, timeout_ms, require_modification } => {
                let result = objectiveai::filesystem::logs::client::subscribe_agent_completion_message_video(&client, &id, message_index, media_index, std::time::Duration::from_millis(timeout_ms), require_modification).await;
                {
            ...
            Commands::Clear => {
                crate::ack::emit_log_clear_count(objectiveai::filesystem::logs::client::clear_agent_completion_message_video(&client).await?);
                Ok(())
            },
        }
    }
