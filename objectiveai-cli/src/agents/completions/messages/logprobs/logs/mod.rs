    ...
    pub async fn handle(self, cli_config: &crate::Config, handle: &objectiveai_sdk::cli::output::Handle) -> Result<(), crate::error::Error> {
        ...
        match self {
            Commands::Get { id, message_index, filter } => {
                let content = client.read_agent_completion_message_logprobs(&id, message_index, filter.as_deref()).await.map(objectiveai_sdk::filesystem::logs::LogContent::Json)?;
                {
            ...
            Commands::Subscribe { id, message_index, timeout_ms, require_modification, filter } => {
                let result = client.subscribe_agent_completion_message_logprobs(&id, message_index, std::time::Duration::from_millis(timeout_ms), require_modification, filter.as_deref()).await?;
                {
            ...
            Commands::Clear => {
                crate::log_line::emit_log_clear_count(client.clear_agent_completion_message_logprobs().await?, handle).await;
                Ok(())
            },
        }
    }
