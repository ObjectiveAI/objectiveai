        ...
        match self {
            Commands::Get {
                ...
                let content = client
                    ...
                    .await
                    .map(crate::filesystem::logs::LogContent::file)?;
                {
            ...
            Commands::Subscribe {
                ...
                let result = client
                    .subscribe_agent_completion_message_file(
                        &id,
                        message_index,
                        media_index,
                    ...
                    .await;
                {
                    match result.map(crate::filesystem::logs::LogContent::file) {
                        Some(content) => {
                            crate::log_line::emit_log_content(content, handle).await;
                            Ok(())
                        }
            ...
            Commands::Clear => {
                crate::log_line::emit_log_clear_count(
                    client.clear_agent_completion_message_files().await?,
                    handle,
                )
                .await;
                Ok(())
