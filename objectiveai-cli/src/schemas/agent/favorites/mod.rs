...
impl Commands {
    pub async fn handle(self, handle: &objectiveai_sdk::cli::output::Handle) -> Result<(), crate::error::Error> {
        match self {
            Commands::List => {
                const NAMES: &[&str] = &["Action", "ChangedNotification"];
            ...
            Commands::Action { .. } => {
                let schema: serde_json::Value = serde_json::from_str(
                    include_str!("../../../../../objectiveai-json-schema/agent.favorites.Action.json"),
            ...
            Commands::ChangedNotification { .. } => {
                let schema: serde_json::Value = serde_json::from_str(
                    include_str!("../../../../../objectiveai-json-schema/agent.favorites.ChangedNotification.json"),
                ).expect("embedded JSON Schema must parse");
                objectiveai_sdk::cli::output::Output::Notification(
                    objectiveai_sdk::cli::output::Notification {
                        agent_id: None,
