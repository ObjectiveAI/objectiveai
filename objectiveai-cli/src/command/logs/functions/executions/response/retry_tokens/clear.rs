//! `logs functions executions response retry_tokens clear` — clear a category of stored log records; returns the
...
pub async fn execute(ctx: &Context, _request: Request) -> Result<Response, Error> {
    let count = ctx.filesystem.clear_function_execution_retry_tokens().await?;
    Ok(Response { count })
}
