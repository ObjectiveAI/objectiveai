//! `logs functions executions response` sub-tier.

use std::pin::Pin;

use futures::{Stream, StreamExt};
use objectiveai_sdk::cli::command::logs::functions::executions::response::{Request, ResponseItem};
...
pub async fn execute(ctx: &Context, request: Request) -> Result<ItemStream, Error> {
    let stream: ItemStream = match request {
        ...
        Request::RetryTokens(req) => {
            let inner = retry_tokens::execute(ctx, req).await?;
