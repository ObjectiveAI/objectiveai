//! `logs agents completions response messages audio get` — read a stored log record from disk.
...
pub async fn execute(ctx: &Context, request: Request) -> Result<Response, Error> {
    Ok(ctx
        .filesystem
        .read_agent_completion_message_audio(&request.id, request.message_index, request.media_index)
        .await?)
}
