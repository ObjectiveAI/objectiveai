//! `logs agents completions response messages clear` — clear a category of stored log records; returns the
...
pub async fn execute(ctx: &Context, _request: Request) -> Result<Response, Error> {
    let count = ctx.filesystem.clear_agent_completion_messages().await?;
    Ok(Response { count })
}
