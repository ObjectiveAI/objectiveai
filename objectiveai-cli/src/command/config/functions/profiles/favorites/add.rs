//! `config functions profiles favorites add` — add a named entry to
//! the function-profiles favorites list in on-disk config.
...
pub async fn execute(ctx: &Context, request: Request) -> Result<Response, Error> {
    ...
    config.functions().profiles().add_favorite(favorite);
