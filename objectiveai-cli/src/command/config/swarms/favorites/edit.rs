//! `config swarms favorites edit` — mutate a named entry in the swarm
//! favorites list. Supports updating the note and the commit-on-path
//! (set or remove); drop and re-add to change remote/owner/repo.

use objectiveai_sdk::RemotePathCommitOptional;
use objectiveai_sdk::cli::command::config::swarms::favorites::edit::{
...
pub async fn execute(ctx: &Context, request: Request) -> Result<Response, Error> {
    ...
    let favorite = config.swarms().edit_favorite(&request.name)?;
