    ...
    pub async fn handle(self, cli_config: &crate::Config, handle: &objectiveai_sdk::cli::output::Handle) -> Result<(), crate::error::Error> {
        ...
        match self {
            Commands::Get { id, filter } => {
                let content = client.read_function_invention(&id, filter.as_deref()).await.map(objectiveai_sdk::filesystem::logs::LogContent::Json)?;
                {
            ...
            Commands::Subscribe { id, timeout_ms, require_modification, filter } => {
                let result = client.subscribe_function_invention(&id, std::time::Duration::from_millis(timeout_ms), require_modification, filter.as_deref()).await?;
                {
            ...
            Commands::List { offset, limit } => {
                crate::log_line::emit_log_list(client.list_function_inventions(offset, limit).await?, handle).await;
                Ok(())
            },
            Commands::Clear => {
                crate::log_line::emit_log_clear_count(client.clear_function_inventions().await?, handle).await;
                Ok(())
            },
        }
    }
