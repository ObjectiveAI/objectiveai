...
#[derive(Subcommand)]
pub enum Commands {
    /// Get a recursive function invention log, optionally filtered with jq
    ...
    /// Clear all recursive function invention logs
    Clear,
}
    ...
    pub async fn handle(
        ...
        match self {
            Commands::Get { id, filter } => {
                let content = client
                    .read_function_invention_recursive(&id, filter.as_deref())
                    .await
            ...
            Commands::Clear => {
                crate::log_line::emit_log_clear_count(
                    client.clear_function_inventions_recursive().await?,
                    handle,
                )
                .await;
                Ok(())
