...
impl Commands {
    pub async fn handle(
        ...
        match self {
            Commands::Get { args } => {
                let (function_path, profile_path) =
                    args.resolve(|| get_favorites(cli_config)).await?;
                ...
                crate::api::run(cli_config, |http_client| async move {
                    let (function, profile) = tokio::join!(
                        ...
                        objectiveai_sdk::functions::profiles::get_profile(
                            &http_client,
                            profile_path
                        ),
                    );
            ...
            Commands::List { source } => match source {
                crate::list::Source::Favorites => {
                    crate::list::pair_favorites(|| get_favorites(cli_config), handle).await
                }
                crate::list::Source::Filesystem => {
                    Err(crate::error::Error::PairsSourceNotSupported("filesystem"))
                }
                crate::list::Source::Objectiveai => {
                    crate::list::pair_single(cli_config, |c| Box::pin(list_objectiveai(c)), handle)
                        .await
                }
                crate::list::Source::Mock => {
                    Err(crate::error::Error::PairsSourceNotSupported("mock"))
                }
