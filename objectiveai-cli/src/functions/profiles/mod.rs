        ...
        match self {
            Commands::Get { args } => {
                ...
                crate::api::run(cli_config, |http_client| async move {
                    let response =
                        objectiveai_sdk::functions::profiles::get_profile(&http_client, path)
                            .await?;
                    objectiveai_sdk::cli::output::Output::Notification(
                        objectiveai_sdk::cli::output::Notification {
                            value: (objectiveai_sdk::cli::output::Profile { profile: response })
                                .into(),
                        },
                    )
                    .emit(&handle)
            ...
            Commands::List { source } => {
                ...
                match source {
                    ...
                    crate::list::Source::Mock => {
                        crate::list::single(
                            cli_config,
                            |c| Box::pin(list_source(c, ListProfilesSource::Mock)),
                            handle,
                        )
                        .await
                    }
