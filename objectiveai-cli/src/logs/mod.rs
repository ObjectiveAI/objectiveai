...
//! paths exactly, with one rename rule applied at the top: certain
//! SDK singular leading segments become plural — `agent/` → `agents/`,
//! `function/` → `functions/`, `profile/` → `profiles/`. `vector/`
//! stays singular. Already-plural segments pass through.
//!
