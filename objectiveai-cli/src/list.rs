    ...
    F: FnOnce(
            ...
            Box<
                dyn std::future::Future<
                        Output = Result<Vec<objectiveai_sdk::RemotePath>, crate::error::Error>,
                    > + Send,
            >,
        > + Send
        + 'static,
{
    ...
    crate::api::run(cli_config, move |http_client| {
        Box::pin(async move {
            let items: Vec<ListItem> = list_remote(http_client)
                .await?
                .into_iter()
                .map(ListItem::Item)
                .collect();
...
fn favorite_matches_path(
    ...
    match (fav_path, path) {
        (
            objectiveai_sdk::RemotePathCommitOptional::Github {
                owner: fo,
                repository: fr,
                commit: fc,
            },
            objectiveai_sdk::RemotePath::Github {
                owner: po,
                repository: pr,
                commit: pc,
            },
        ...
        (
            objectiveai_sdk::RemotePathCommitOptional::Filesystem {
                owner: fo,
                repository: fr,
                commit: fc,
            },
            objectiveai_sdk::RemotePath::Filesystem {
                owner: po,
                repository: pr,
                commit: pc,
            },
...
where
    F: FnOnce(
            ...
            Box<
                dyn std::future::Future<
                        Output = Result<
                            Vec<objectiveai_sdk::functions::response::ListFunctionProfilePairItem>,
                            crate::error::Error,
                        >,
                    > + Send,
            >,
        ...
        + 'static,
{
    ...
    crate::api::run(cli_config, move |http_client| {
        Box::pin(async move {
            let items: Vec<PairListItem> = list_remote(http_client)
                .await?
                .into_iter()
    ...
    crate::api::run(cli_config, move |http_client| {
        Box::pin(async move {
            ...
            for item in oai_items {
                if !favorites
                    .iter()
                    .any(|fav| pair_favorite_matches(fav, &item))
                {
