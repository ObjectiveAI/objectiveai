#!/usr/bin/env bash
# Install the hello-plugin fixture into the shared test OBJECTIVEAI_DIR.
#
# Expects the binary to be already built (build.sh, or `cargo build -p
# hello-plugin`). Copies it into the coordinate's cli/ dir and writes the
...
VDIR="$OBJECTIVEAI_DIR/bin/plugins/objectiveai/hello/0.0.1"
mkdir -p "$VDIR/cli"
...
JSON
echo "install: plugins/objectiveai/hello/0.0.1 (${BIN}${EXE}, ${PROFILE})"
