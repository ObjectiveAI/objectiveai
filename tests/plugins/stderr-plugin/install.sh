#!/usr/bin/env bash
# Install the stderr-plugin fixture into the shared test OBJECTIVEAI_DIR.
#
# Expects the binary to be already built (build.sh, or `cargo build -p
# stderr-plugin`). Copies it into the coordinate's cli/ dir and writes the
...
VDIR="$OBJECTIVEAI_DIR/bin/plugins/objectiveai/stderr/0.0.1"
mkdir -p "$VDIR/cli"
...
cat > "$VDIR/objectiveai.json" <<JSON
{
  ...
  "name": "stderr",
  "version": "0.0.1",
  "description": "E2E test fixture: writes 3 lines to stderr",
  "exec": {
    "windows": ["./${BIN}.exe"],
    "linux": ["./${BIN}"],
    "macos": ["./${BIN}"]
...
JSON
echo "install: plugins/objectiveai/stderr/0.0.1 (${BIN}${EXE}, ${PROFILE})"
