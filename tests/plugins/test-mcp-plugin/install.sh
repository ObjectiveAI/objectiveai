#!/usr/bin/env bash
# Install the test-mcp-plugin fixture into the shared test OBJECTIVEAI_DIR.
#
# Expects the binary to be already built (build.sh, or `cargo build -p
# test-mcp-plugin`). Copies it into the coordinate's cli/ dir and writes
# the matching objectiveai.json. Self-contained on purpose — each
...
VDIR="$OBJECTIVEAI_DIR/bin/plugins/testorg/test-mcp-plugin/1.0.0"
mkdir -p "$VDIR/cli"
...
JSON
echo "install: plugins/testorg/test-mcp-plugin/1.0.0 (${BIN}${EXE}, ${PROFILE})"
