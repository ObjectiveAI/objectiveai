# Report 11 — rotating OAuth state as resources

From the pinned source (`sources/hermes-agent`, v0.20.6). Report 10
initially dropped the four rotating-OAuth providers; that was
wrong-way-round. The POINT of auth-as-arguments is that rotation
becomes supportable: the caller hands the current OAuth state in as
a RESOURCE (`*_resource` fields on the provider structures), the
run rotates it, and the rotated state is surfaced back so the
