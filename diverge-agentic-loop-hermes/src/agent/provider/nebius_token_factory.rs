...
/// Nebius Token Factory.
///
/// APPLICATION: the harness sets `NEBIUS_API_KEY` to
/// [`api_key`](Self::api_key) in the gateway's process environment
/// before Hermes starts.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, JsonSchema)]
pub struct Provider {
    /// The discriminator. Always `nebius-token-factory`.
    pub provider: NebiusTokenFactory,
    /// The API key, applied as `NEBIUS_API_KEY`.
    pub api_key: String,
}
