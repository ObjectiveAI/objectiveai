...
//! value this image accepts. The run itself is [`run::run`]: the
...
/// is never refused); an agent value that is not this image's
/// (`400`); an empty prompt (`400` — a turn needs one); a database
