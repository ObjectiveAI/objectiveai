#!/usr/bin/env bash
# Publishes objectiveai-mcp-proxy to crates.io as the `objectiveai-mcp-proxy` crate.
#
# Usage:
#   bash objectiveai-mcp-proxy/publish.sh                # crates.io (via GHA)
#   bash objectiveai-mcp-proxy/publish.sh --build-only   # local cargo publish --dry-run
#
# `--test` is not supported (crates.io has no test registry).
#
# Output is captured to .logs/publish/objectiveai-mcp-proxy.txt.
#
# Setup (one-time):
