...
//! read side matching the [`crate::daemon::Client`] executor's
//! write side.
//!
...
//! otherwise.
//!
//! [`CommandListener`] IS a `Stream`: it yields one [`ListenerExecution`] envelope
...
/// The root run stream — see the module docs. Minted by
/// [`Client::command_listener`](crate::daemon::Client::command_listener);
/// returned only once the stream has OPENED (connection/auth failures
/// surface at connect, not as a first `Err` item).
...
impl CommandListener {
    ...
    /// pump. The returned [`CommandListener`] is the root envelope
    /// stream.
    pub(crate) async fn connect(
        client: &crate::daemon::Client,
    ) -> Result<CommandListener, Error> {
        let source = client.open_sse("/listen").await?;
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel::<Result<ListenerExecution, Error>>();
        tokio::spawn(pump(source, tx));
        Ok(CommandListener { rx })
    }
}

impl Stream for CommandListener {
...
async fn pump(
    ...
    loop {
        match source.next().await {
            ...
            // reconnects); a final `Err` surfaces first, carrying the
            // transport error whole (its `source()` chain intact —
            // reset vs premature close vs framing).
            Some(Err(e)) => {
                let _ = tx.send(Err(Error::Open(e)));
                break;
            }
            None => break,
        }
