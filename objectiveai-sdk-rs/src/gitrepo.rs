//! Git checkouts of plugin repos: fetch exactly ONE tag into a
//! throwaway dir under a caller-owned temp root. Shared by the
//! laboratory host (container-image builds, `<bin>/temp/daemon`) and
//! the viewer (extension installs, `<bin>/temp/viewer`) — each caller
//! owns its temp subtree and sweeps it at boot.
//!
//! The source is LOCAL-FIRST: if `<plugins_dir>/<owner>/<name>` holds
//! a git repo containing the tag, the fetch pulls from that repo (how
//! local plugin development works — no push to GitHub needed);
...
//! them after use (success AND failure), and each caller's boot sweep
//! ([`sweep_temp`]) clears any leftovers from a hard-killed
//! predecessor.
...
pub struct CheckedOutRepo {
    /// The checkout root — a fresh `<temp_dir>/<uuid>` dir.
    pub dir: PathBuf,
    /// The tag's commit SHA (hex).
    pub commit_sha: String,
}

/// Shallow-fetch `refs/tags/{tag}` of the plugin repo — the local
/// `<plugins_dir>/<owner>/<name>` repo when it holds the tag,
/// `https://github.com/{owner}/{name}.git` otherwise — into a fresh
/// `<temp_dir>/<uuid>` dir and check it out detached. `plugins_dir`
/// is the local override root (`<objectiveai_dir>/plugins`); `None`
/// fetches from GitHub only. The caller owns the returned dir and
/// MUST [`remove_checkout`] it when done — this function only cleans
/// up after its own failures.
///
...
pub async fn fetch_at_tag(
    ...
    let result = {
        let dir = dir.clone();
        let plugins_dir = plugins_dir.map(Path::to_path_buf);
...
/// Boot-sweep pass: delete EVERY leftover under `temp_dir` — a
/// hard-killed predecessor's checkouts. The caller passes its OWN
/// temp subtree (`<bin>/temp/daemon`, `<bin>/temp/viewer`) — the
/// subtrees are private by contract, so sweeps never cross. Runs
/// before the caller serves anything; new checkouts mint fresh uuid
/// dirs, so nothing races it. Best-effort by design.
pub async fn sweep_temp(temp_dir: &Path) {
