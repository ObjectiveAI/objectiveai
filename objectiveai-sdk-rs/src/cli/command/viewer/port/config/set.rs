...
pub mod request_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["viewer", "port", "config", "set", "--request-schema"].into_iter().map(String::from).collect()
        }
    }
...
pub mod response_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["viewer", "port", "config", "set", "--response-schema"].into_iter().map(String::from).collect()
        }
    }
