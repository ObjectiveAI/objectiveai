//! `development plugins viewer create` — point a plugin's VIEWER
//! half at a local directory.
//!
//! The viewer then serves `plugin://` for these coordinates straight
//! from that directory's `viewer.development.output` — no container
//! build, no install — and hot-reloads open tabs as the files
//! change. Registrations live in the resident daemon's memory and
//! are pushed to the viewer over its stdin; they do not survive a
//! daemon restart.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.development.plugins.viewer.create.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.development.plugins.viewer.create.Path")]
pub enum Path {
    #[serde(rename = "development/plugins/viewer/create")]
    DevelopmentPluginsViewerCreate,
}
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.development.plugins.viewer.create.Response")]
pub struct Response {
    pub owner: String,
    pub name: String,
    pub version: String,
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            path_type: Path::DevelopmentPluginsViewerCreate,
            owner,
            name,
            version,
            path,
...
/// One `/listen` broadcast run of `development plugins viewer create`.
