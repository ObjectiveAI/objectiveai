//! `channels logs request` — the PUBLISHER→owner write. Append a
//! `request` entry (arbitrary-JSON `content`) to the channel log.
//! Requires the publisher secret (`S_pub`) and an OPEN channel.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.channels.logs.request.Request")]
pub struct Request {
    pub path_type: Path,
    pub channel_id: String,
    /// The publisher secret (`S_pub`).
    pub secret: String,
...
#[schemars(rename = "cli.command.channels.logs.request.Path")]
pub enum Path {
    #[serde(rename = "channels/logs/request")]
    ChannelsLogsRequest,
}
...
#[schemars(rename = "cli.command.channels.logs.request.Response")]
pub enum Response {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            path_type: Path::ChannelsLogsRequest,
            channel_id,
            secret,
            content,
            base: args.base.into(),
