//! `daemon config address set` — async handler stub.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.daemon.config.address.set.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.daemon.config.address.set.Path")]
pub enum Path {
    #[serde(rename = "daemon/config/address/set")]
    DaemonConfigAddressSet,
}
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            base: args.base.into(), path_type: Path::DaemonConfigAddressSet,
            scope,
...
/// One `/listen` broadcast run of `mcp config address set`: the actual
