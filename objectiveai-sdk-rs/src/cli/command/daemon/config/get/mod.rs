//! `daemon config get` — async handler stub.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.daemon.config.get.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.daemon.config.get.Path")]
pub enum Path {
    #[serde(rename = "daemon/config/get")]
    DaemonConfigGet,
}
...
#[schemars(rename = "cli.command.daemon.config.get.Response")]
pub struct Response {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::DaemonConfigGet,
            base: args.base.into(),
        })
    }
}
...
/// One `/listen` broadcast run of `mcp config get`: the actual
