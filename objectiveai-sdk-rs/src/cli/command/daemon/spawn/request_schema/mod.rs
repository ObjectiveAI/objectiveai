...
#[schemars(rename = "cli.command.daemon.spawn.request_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.daemon.spawn.request_schema.Path")]
pub enum Path {
    #[serde(rename = "daemon/spawn/request_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::DaemonSpawnRequestSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `daemon spawn request_schema`: the actual
