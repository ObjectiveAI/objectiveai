...
#[schemars(rename = "cli.command.daemon.kill.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.daemon.kill.response_schema.Path")]
pub enum Path {
    #[serde(rename = "daemon/kill/response_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::DaemonKillResponseSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `daemon kill response_schema`: the actual
