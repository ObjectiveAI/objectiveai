//! `laboratories config addresses del` — async handler stub.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.laboratories.config.addresses.del.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.laboratories.config.addresses.del.Path")]
pub enum Path {
    #[serde(rename = "laboratories/config/addresses/del")]
    LaboratoriesConfigAddressesDel,
}
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self {
            base: args.base.into(), path_type: Path::LaboratoriesConfigAddressesDel,
...
/// One `/listen` broadcast run of `laboratories config addresses del`: the actual
