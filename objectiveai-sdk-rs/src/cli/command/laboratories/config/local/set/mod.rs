//! `laboratories config local set` — async handler stub.

use crate::cli::command::CommandRequest;

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.laboratories.config.local.set.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.laboratories.config.local.set.Path")]
pub enum Path {
    #[serde(rename = "laboratories/config/local/set")]
    LaboratoriesConfigLocalSet,
}
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            base: args.base.into(), path_type: Path::LaboratoriesConfigLocalSet,
            scope,
...
/// One `/listen` broadcast run of `laboratories config local set`: the actual
