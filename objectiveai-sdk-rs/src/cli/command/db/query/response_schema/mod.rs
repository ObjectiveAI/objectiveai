...
#[schemars(rename = "cli.command.db.query.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.db.query.response_schema.Path")]
pub enum Path {
    #[serde(rename = "db/query/response_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self {
            path_type: Path::DbQueryResponseSchema,
            base: args.base.into(),
        })
    }
}
...
/// One `/listen` broadcast run of `db query response_schema`: the actual
