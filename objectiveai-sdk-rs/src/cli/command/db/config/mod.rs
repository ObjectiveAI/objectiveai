...
> {
    ...
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<Response, E::Error>> + Send>> =
        match request {
            ...
            Request::User(req) => {
                let inner = user::execute(executor, req, identity).await?;
                Box::pin(inner.map(|r| r.map(Response::User)))
            }
            Request::Password(req) => {
                let inner = password::execute(executor, req, identity).await?;
                Box::pin(inner.map(|r| r.map(Response::Password)))
            }
            Request::Database(req) => {
                let inner = database::execute(executor, req, identity).await?;
...
> {
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<serde_json::Value, E::Error>> + Send>> =
        match request {
            ...
            Request::User(req) => {
                let inner = user::execute_transform(executor, req, transform, identity).await?;
                Box::pin(inner)
            }
            Request::Password(req) => {
                let inner = password::execute_transform(executor, req, transform, identity).await?;
                Box::pin(inner)
            }
            Request::Database(req) => {
                let inner = database::execute_transform(executor, req, transform, identity).await?;
                Box::pin(inner)
            }
        };
    Ok(stream)
