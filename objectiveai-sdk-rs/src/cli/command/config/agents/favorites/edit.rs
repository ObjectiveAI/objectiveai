//! `config agents favorites edit` — async handler stub.
...
pub enum Path {
    #[serde(rename = "config/agents/favorites/edit")]
    ConfigAgentsFavoritesEdit,
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.config.agents.favorites.RequestCommitChange")]
...
pub mod request_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/agents/favorites/edit/request_schema")]
        ConfigAgentsFavoritesEditRequestSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "agents", "favorites", "edit", "request-schema"].into_iter().map(String::from).collect();
...
pub mod response_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/agents/favorites/edit/response_schema")]
        ConfigAgentsFavoritesEditResponseSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "agents", "favorites", "edit", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
