//! `config agents favorites add` — async handler stub.
...
pub mod request_schema {
    ...
    pub enum Path {
        #[serde(rename = "config/agents/favorites/add/request_schema")]
        ConfigAgentsFavoritesAddRequestSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "agents", "favorites", "add", "request-schema"].into_iter().map(String::from).collect();
...
pub mod response_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/agents/favorites/add/response_schema")]
        ConfigAgentsFavoritesAddResponseSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "agents", "favorites", "add", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
