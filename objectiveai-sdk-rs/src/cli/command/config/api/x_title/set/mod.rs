...
#[schemars(rename = "cli.command.config.api.x_title.set.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.api.x_title.set.Path")]
pub enum Path {
    #[serde(rename = "config/api/x_title/set")]
    ConfigApiXTitleSet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "api".to_string(), "x-title".to_string(), "set".to_string(), self.value.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigApiXTitleSet,
            scope,
            value: args.value,
        })
    }
