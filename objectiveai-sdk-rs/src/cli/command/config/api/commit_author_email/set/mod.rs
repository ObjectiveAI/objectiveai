...
#[schemars(rename = "cli.command.config.api.commit_author_email.set.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.api.commit_author_email.set.Path")]
pub enum Path {
    #[serde(rename = "config/api/commit_author_email/set")]
    ConfigApiCommitAuthorEmailSet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "api".to_string(), "commit-author-email".to_string(), "set".to_string(), self.value.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigApiCommitAuthorEmailSet,
            scope,
            value: args.value,
        })
    }
