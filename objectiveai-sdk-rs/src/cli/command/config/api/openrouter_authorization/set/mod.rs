...
#[schemars(rename = "cli.command.config.api.openrouter_authorization.set.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.api.openrouter_authorization.set.Path")]
pub enum Path {
    #[serde(rename = "config/api/openrouter_authorization/set")]
    ConfigApiOpenrouterAuthorizationSet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "api".to_string(), "openrouter-authorization".to_string(), "set".to_string(), self.value.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigApiOpenrouterAuthorizationSet,
            scope,
            value: args.value,
        })
    }
