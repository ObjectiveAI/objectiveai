...
#[schemars(rename = "cli.command.config.api.mcp_authorization.del.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.api.mcp_authorization.del.Path")]
pub enum Path {
    #[serde(rename = "config/api/mcp_authorization/del")]
    ConfigApiMcpAuthorizationDel,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "api".to_string(), "mcp-authorization".to_string(), "del".to_string(), self.key.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigApiMcpAuthorizationDel,
            scope,
            key: args.key,
        })
    }
