...
pub mod request_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/swarms/favorites/add/request_schema")]
        ConfigSwarmsFavoritesAddRequestSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "swarms", "favorites", "add", "request-schema"].into_iter().map(String::from).collect();
    ...
    impl TryFrom<Args> for Request {
        type Error = crate::cli::command::FromArgsError;
        fn try_from(args: Args) -> Result<Self, Self::Error> {
            Ok(Self { path_type: Path::ConfigSwarmsFavoritesAddRequestSchema, jq: args.jq })
        }
    }
...
pub mod response_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/swarms/favorites/add/response_schema")]
        ConfigSwarmsFavoritesAddResponseSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "swarms", "favorites", "add", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
    ...
    impl TryFrom<Args> for Request {
        type Error = crate::cli::command::FromArgsError;
        fn try_from(args: Args) -> Result<Self, Self::Error> {
            Ok(Self { path_type: Path::ConfigSwarmsFavoritesAddResponseSchema, jq: args.jq })
        }
    }
