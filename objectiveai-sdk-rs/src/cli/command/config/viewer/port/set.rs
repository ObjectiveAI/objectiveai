...
pub enum Path {
    #[serde(rename = "config/viewer/port/set")]
    ConfigViewerPortSet,
}
...
pub mod request_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/viewer/port/set/request_schema")]
        ConfigViewerPortSetRequestSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "viewer", "port", "set", "request-schema"].into_iter().map(String::from).collect();
...
pub mod response_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/viewer/port/set/response_schema")]
        ConfigViewerPortSetResponseSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "viewer", "port", "set", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
