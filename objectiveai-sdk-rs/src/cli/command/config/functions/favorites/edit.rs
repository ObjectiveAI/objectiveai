//! `config functions favorites edit` — async handler stub.
...
pub enum Path {
    #[serde(rename = "config/functions/favorites/edit")]
    ConfigFunctionsFavoritesEdit,
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.config.functions.favorites.RequestCommitChange")]
...
impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "functions".to_string(), "favorites".to_string(), "edit".to_string(), self.name.clone()];
...
pub mod request_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/functions/favorites/edit/request_schema")]
        ConfigFunctionsFavoritesEditRequestSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "functions", "favorites", "edit", "request-schema"].into_iter().map(String::from).collect();
...
pub mod response_schema {
    ...
    #[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
    pub enum Path {
        #[serde(rename = "config/functions/favorites/edit/response_schema")]
        ConfigFunctionsFavoritesEditResponseSchema,
    }
    #[derive(clap::Args)]
    pub struct Args {
        #[arg(long)]
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["config", "functions", "favorites", "edit", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
