...
impl TryFrom<Command> for Request {
    ...
    fn try_from(command: Command) -> Result<Self, Self::Error> {
        match command {
            ...
            Command::Remote { command } =>
                Ok(Request::Remote(remote::Request::try_from(command)?)),
        }
    }
}
...
> {
    ...
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<Response, E::Error>> + Send>> =
        match request {
            ...
            Request::Remote(req) => {
                let inner = remote::execute(executor, req, agent_arguments).await?;
                Box::pin(inner.map(|r| r.map(Response::Remote)))
            }
        };
    Ok(stream)
}
...
> {
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<serde_json::Value, E::Error>> + Send>> =
        match request {
            ...
            Request::Remote(req) => {
                let inner = remote::execute_jq(executor, req, jq, agent_arguments).await?;
                Box::pin(inner)
            }
        };
    Ok(stream)
