...
#[schemars(rename = "cli.command.config.functions.profiles.pairs.favorites.del.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.functions.profiles.pairs.favorites.del.Path")]
pub enum Path {
...
impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "functions".to_string(), "profiles".to_string(), "pairs".to_string(), "favorites".to_string(), "del".to_string(), self.name.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigFunctionsProfilesPairsFavoritesDel, scope, name: args.name })
    }
}
