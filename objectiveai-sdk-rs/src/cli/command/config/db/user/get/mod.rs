...
#[schemars(rename = "cli.command.config.db.user.get.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.db.user.get.Path")]
pub enum Path {
    #[serde(rename = "config/db/user/get")]
    ConfigDbUserGet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "db".to_string(), "user".to_string(), "get".to_string()];
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.config.db.user.get.Response")]
pub struct Response {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigDbUserGet,
            scope,
            jq: args.jq,
        })
    }
