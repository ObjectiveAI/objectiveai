...
#[schemars(rename = "cli.command.config.db.password.get.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.db.password.get.Path")]
pub enum Path {
    #[serde(rename = "config/db/password/get")]
    ConfigDbPasswordGet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "db".to_string(), "password".to_string(), "get".to_string()];
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.config.db.password.get.Response")]
pub struct Response {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigDbPasswordGet,
            scope,
            jq: args.jq,
        })
    }
