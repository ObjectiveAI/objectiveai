...
#[schemars(rename = "cli.command.config.db.address.set.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.config.db.address.set.Path")]
pub enum Path {
    #[serde(rename = "config/db/address/set")]
    ConfigDbAddressSet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["config".to_string(), "db".to_string(), "address".to_string(), "set".to_string(), self.value.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::ConfigDbAddressSet,
            scope,
            value: args.value,
        })
    }
