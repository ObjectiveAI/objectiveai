...
> {
    ...
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<ResponseItem, E::Error>> + Send>> =
        match request {
            Request::Execute(req) => {
                let inner = execute::execute(executor, req, identity).await?;
            ...
            Request::Profiles(req) => {
                let inner = profiles::execute(executor, req, identity).await?;
...
> {
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<serde_json::Value, E::Error>> + Send>> =
        match request {
            Request::Execute(req) => {
                let inner = execute::execute_transform(executor, req, transform, identity).await?;
                Box::pin(inner)
            }
            Request::Get(req) => {
            ...
            Request::Profiles(req) => {
                let inner = profiles::execute_transform(executor, req, transform, identity).await?;
                Box::pin(inner)
            }
            Request::Publish(req) => {
...
pub enum ListenerExecution {
    Execute(execute::ListenerExecution),
    ...
    Profiles(profiles::ListenerExecution),
