...
pub mod request_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["functions", "profiles", "favorites", "config", "del", "--request-schema"].into_iter().map(String::from).collect()
        }
    }
...
pub mod response_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["functions", "profiles", "favorites", "config", "del", "--response-schema"].into_iter().map(String::from).collect()
        }
    }
