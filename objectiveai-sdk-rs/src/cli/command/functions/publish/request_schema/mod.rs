...
#[schemars(rename = "cli.command.functions.publish.request_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.functions.publish.request_schema.Path")]
pub enum Path {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::FunctionsPublishRequestSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `functions publish request_schema`: the actual
