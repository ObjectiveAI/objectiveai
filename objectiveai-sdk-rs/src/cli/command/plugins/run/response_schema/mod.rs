...
#[schemars(rename = "cli.command.plugins.run.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.plugins.run.response_schema.Path")]
pub enum Path {
    #[serde(rename = "plugins/run/response_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::PluginsRunResponseSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `plugins run response_schema`: the actual
