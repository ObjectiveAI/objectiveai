...
#[schemars(rename = "cli.command.plugins.run.request_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.plugins.run.request_schema.Path")]
pub enum Path {
    #[serde(rename = "plugins/run/request_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::PluginsRunRequestSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `plugins run request_schema`: the actual
