...
#[schemars(rename = "cli.command.plugins.install.github.request_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.plugins.install.github.request_schema.Path")]
pub enum Path {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::PluginsInstallGithubRequestSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `plugins install github request_schema`: the actual
