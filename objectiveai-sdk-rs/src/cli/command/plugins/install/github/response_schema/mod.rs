...
#[schemars(rename = "cli.command.plugins.install.github.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.plugins.install.github.response_schema.Path")]
pub enum Path {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::PluginsInstallGithubResponseSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `plugins install github response_schema`: the actual
