...
pub mod request_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["logs", "functions", "executions", "response", "subscribe", "--request-schema"].into_iter().map(String::from).collect()
        }
    }
...
pub mod response_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["logs", "functions", "executions", "response", "subscribe", "--response-schema"].into_iter().map(String::from).collect()
        }
    }
