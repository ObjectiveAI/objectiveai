...
#[schemars(rename = "cli.command.logs.agents.completions.request.notifications.text.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.logs.agents.completions.request.notifications.text.Path")]
pub enum Path {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::LogsAgentsCompletionsRequestNotificationsTextGet,
...
pub mod request_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["logs", "agents", "completions", "request", "notifications", "text", "get", "request-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
    ...
    impl TryFrom<Args> for Request {
        type Error = crate::cli::command::FromArgsError;
        fn try_from(args: Args) -> Result<Self, Self::Error> {
            Ok(Self { path_type: Path::LogsAgentsCompletionsRequestNotificationsTextGetRequestSchema, jq: args.jq })
        }
    }
...
pub mod response_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            let mut argv: Vec<String> = vec!["logs", "agents", "completions", "request", "notifications", "text", "get", "response-schema"].into_iter().map(String::from).collect();
            if let Some(jq) = &self.jq {
                argv.push("--jq".to_string());
                argv.push(jq.clone());
            }
    ...
    impl TryFrom<Args> for Request {
        type Error = crate::cli::command::FromArgsError;
        fn try_from(args: Args) -> Result<Self, Self::Error> {
            Ok(Self { path_type: Path::LogsAgentsCompletionsRequestNotificationsTextGetResponseSchema, jq: args.jq })
        }
    }
