...
pub enum ResponseItem {
    ...
    #[schemars(title = "Continuations")]
    Continuations(continuations::Response),
    #[schemars(title = "Get")]
    Get(get::Response),
    #[schemars(title = "GetRequestSchema")]
...
impl crate::cli::command::CommandResponse for ResponseItem {
    fn into_mcp(self) -> crate::cli::command::McpResponseItem {
        match self {
            ...
            ResponseItem::Continuations(v) => v.into_mcp(),
            ...
            ResponseItem::Messages(v) => v.into_mcp(),
...
impl TryFrom<Command> for Request {
    ...
    fn try_from(command: Command) -> Result<Self, Self::Error> {
        match command {
            ...
            Command::Continuations { command } =>
                Ok(Request::Continuations(continuations::Request::try_from(command)?)),
...
impl crate::cli::command::CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        match self {
            ...
            Request::Continuations(inner) => inner.into_command(),
    ...
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<ResponseItem, E::Error>> + Send>> =
        match request {
            ...
            Request::Continuations(req) => {
                let inner = continuations::execute(executor, req, agent_arguments).await?;
...
> {
    let stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<serde_json::Value, E::Error>> + Send>> =
        match request {
            ...
            Request::Continuations(req) => {
                let inner = continuations::execute_jq(executor, req, jq, agent_arguments).await?;
                Box::pin(inner)
            }
            Request::Get(req) => {
