...
pub mod request_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["logs", "agents", "completions", "response", "messages", "tool_calls", "get", "--request-schema"].into_iter().map(String::from).collect()
        }
    }
...
pub mod response_schema {
    ...
    impl CommandRequest for Request {
        fn into_command(&self) -> Vec<String> {
            vec!["logs", "agents", "completions", "response", "messages", "tool_calls", "get", "--response-schema"].into_iter().map(String::from).collect()
        }
    }
