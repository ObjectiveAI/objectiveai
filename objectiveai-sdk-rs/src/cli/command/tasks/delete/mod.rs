//! `tasks delete` — remove a task by id. Idempotent: deleting an
//! absent task reports `deleted: false`, never an error. A task
//! deleted mid-run finishes its in-flight run but records nothing and
//! never fires again.
