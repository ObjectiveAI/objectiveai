...
#[schemars(rename = "cli.command.mcp.config.address.set.Request")]
pub struct Request {
    pub path_type: Path,
...
#[schemars(rename = "cli.command.mcp.config.address.set.Path")]
pub enum Path {
    #[serde(rename = "mcp/config/address/set")]
    McpConfigAddressSet,
}

impl CommandRequest for Request {
    fn into_command(&self) -> Vec<String> {
        let mut argv = vec!["mcp".to_string(), "config".to_string(), "address".to_string(), "set".to_string(), self.value.clone()];
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        Ok(Self { path_type: Path::McpConfigAddressSet,
            scope,
            value: args.value,
        })
    }
