...
#[schemars(rename = "cli.command.python.request_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.python.request_schema.Path")]
pub enum Path {
    #[serde(rename = "python/request_schema")]
    PythonRequestSchema,
}
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self {
            path_type: Path::PythonRequestSchema,
            base: args.base.into(),
        })
    }
}
...
/// One `/listen` broadcast run of `python request_schema`: the actual
