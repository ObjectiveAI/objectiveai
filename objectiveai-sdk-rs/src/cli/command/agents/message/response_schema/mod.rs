...
#[schemars(rename = "cli.command.agents.message.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.agents.message.response_schema.Path")]
pub enum Path {
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self { path_type: Path::AgentsMessageResponseSchema, base: args.base.into() })
    }
}
...
/// One `/listen` broadcast run of `agents message response_schema`: the actual
