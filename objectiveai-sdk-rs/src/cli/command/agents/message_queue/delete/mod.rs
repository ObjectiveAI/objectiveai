//! `agents message-queue delete` — manually drop one queued prompt
//! by its `prompts.id` (the id surfaced by `agents message-queue
...
#[schemars(rename = "cli.command.agents.message_queue.delete.Request")]
pub struct Request {
    pub path_type: Path,
    pub id: i64,
...
#[schemars(rename = "cli.command.agents.message_queue.delete.Path")]
pub enum Path {
    #[serde(rename = "agents/message-queue/delete")]
    AgentsMessageQueueDelete,
}
...
/// `RichContent` body.
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.message_queue.delete.Response")]
pub struct Response {
    pub id: i64,
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self {
            path_type: Path::AgentsMessageQueueDelete,
            id: args.id,
            jq: args.jq,
        })
    }
