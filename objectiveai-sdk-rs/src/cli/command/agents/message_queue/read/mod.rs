...
#[serde(untagged)]
#[schemars(rename = "cli.command.agents.message_queue.read.Request")]
pub enum Request {
    #[schemars(title = "Id")]
    Id(id::Request),
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.message_queue.read.ResponseItem")]
#[serde(untagged)]
...
> {
    ...
    > = match request {
        ...
        Request::IdRequestSchema(req) => {
            let value =
                id::request_schema::execute_jq(executor, req, jq, agent_arguments).await?;
            Box::pin(crate::cli::command::StreamOnce::new(Ok(value)))
        }
        Request::IdResponseSchema(req) => {
            let value =
