//! `agents message-queue` — deferred prompts queue. Three top-level subcommands:
//!
...
//!   (tagged by `type`).
//!
//! Drain is no longer a CLI verb — the API consumes queue rows
//! directly via the WS reverse-attach `read_message_queue` /
//! `clear_message_queue` server requests once a matching hierarchy
//! comes online.
