//! `agents mcp tools list` — run `tools/list` against the per-`response_id`
//! MCP listener socket and return the MCP `ListToolsResult`. `--params`
//! is the MCP `ListToolsRequest`, supplied as a JSON string (e.g.
//! `{}` or `{"cursor":"..."}`).
...
#[schemars(rename = "cli.command.agents.mcp.tools.list.Path")]
pub enum Path {
    #[serde(rename = "agents/mcp/tools/list")]
    AgentsMcpToolsList,
}
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.mcp.tools.list.ViewerRequest")]
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.mcp.tools.list.ViewerResponse")]
