//! `agents mcp tools call` — run `tools/call` against the per-`response_id`
//! MCP listener socket and return the MCP `CallToolResult`. `--params`
//! is the MCP `CallToolRequestParams`, supplied as a JSON string.
...
#[schemars(rename = "cli.command.agents.mcp.tools.call.Path")]
pub enum Path {
    #[serde(rename = "agents/mcp/tools/call")]
    AgentsMcpToolsCall,
}
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.mcp.tools.call.ViewerRequest")]
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
#[schemars(rename = "cli.command.agents.mcp.tools.call.ViewerResponse")]
