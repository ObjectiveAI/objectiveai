...
#[schemars(rename = "cli.command.agents.tags.lookup.response_schema.Request")]
pub struct Request {
    pub path_type: Path,
    #[serde(flatten)]
...
#[schemars(rename = "cli.command.agents.tags.lookup.response_schema.Path")]
pub enum Path {
    #[serde(rename = "agents/tags/lookup/response_schema")]
...
impl TryFrom<Args> for Request {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        Ok(Self {
            path_type: Path::AgentsTagsLookupResponseSchema,
            base: args.base.into(),
        })
    }
}
...
/// One `/listen` broadcast run of `agents tags lookup response_schema`: the actual
