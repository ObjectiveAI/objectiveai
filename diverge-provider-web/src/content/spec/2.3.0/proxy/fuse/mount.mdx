---
title: '/fuse/mount'
summary: 'One FUSE mount, made on the server’s request, which names the path, the id, whether the mount is read-only, and whether it is a file or a directory; the proxy sends exactly one answer once the mount is serving, or an error, and closes the connection.'
order: 0
draft: false
---
...
`diverge-provider-sdk-rs/src/container_proxy/fuse/mount/request/request.rs`:

```rust include=diverge-provider-sdk-rs/src/container_proxy/fuse/mount/request/request.rs
```
