...
`diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/request/request.rs`:

```rust include=diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/request/request.rs
```

The messages the proxy sends are defined by
`diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/response/frame.rs`:

```rust include=diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/response/frame.rs
```

The ways in which such a message fails to decode are defined by
`diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/response/error.rs`:

```rust include=diverge-provider-sdk-rs/src/container_proxy/filesystem/tree/response/error.rs
```

The filetree frame after the first byte `0` is defined by
`diverge-provider-sdk-rs/src/shared/filetree/response/frame.rs`:

```rust include=diverge-provider-sdk-rs/src/shared/filetree/response/frame.rs
```

Its nodes are defined by `diverge-provider-sdk-rs/src/shared/filetree/response/node.rs`:
