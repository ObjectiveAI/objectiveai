...
- **The listing.** A payload whose first byte is `0` carries the
  ...
  UTF-8, followed by `bytes` as a varint, followed by `bytes_used`
  as a varint, followed by `created` as a varint, followed by
  `dirhash` as a varint byte length followed by that many bytes of
  UTF-8. A count of `0` is the empty listing, and the payload is then
  exactly the one byte `0`.
...
- **`bytes`.** The size of the volume, in bytes. For a volume the
  ...
  a later `volumes::edit` request.
- **`bytes_used`.** The number of bytes of the volume in use, at the
  time of the response. No frame of this specification reports a
  change in it.
