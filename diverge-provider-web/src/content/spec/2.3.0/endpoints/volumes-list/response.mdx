...
- **The listing.** A payload whose first byte is `0` carries the
  ...
  varint, followed by `tree` as one byte, `0` or `1`. A count of `0`
  is the empty listing, and the payload is then exactly the one byte
  `0`.
...
- **`tree`.** Whether a filetree of a container the volume is mounted
  in covers the mount: `1`, the mount is in the tree and a change
  under it is reported; `0`, the tree leaves the mount out and
  reports nothing under it. The server determines it per volume, and
  it does not change for the life of the volume.
