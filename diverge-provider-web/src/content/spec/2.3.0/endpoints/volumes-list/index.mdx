---
title: 'volumes::list'
summary: 'Tag 3: a client asks which volumes are available to it; the server sends exactly one response carrying every such volume by name, size, use and age, or an error, and the scope finishes.'
order: 3
draft: false
---
...
- **The listing.** The listing is zero or more volumes. The listing
  contains every volume that the client created by a `volumes::create`
  request and has not deleted by a `volumes::delete` request. The
  listing may contain other volumes; which other volumes it contains
  ...
  listing is one the client cannot name.
