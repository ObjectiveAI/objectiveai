...
- **The request.** The tag byte `3` followed by the address and the
  authorization as JSON. [Request](/endpoints/containers-tools-run/server/authorize/request/)
  states its form.
- **The response.** Exactly one channel response of one byte and the
  channel response finish. [Response](/endpoints/containers-tools-run/server/authorize/response/)
  states the sequence.
