...
- **The channels.** The connector opens the channels that
  [Client Channels](/endpoints/containers-tools-connect/client/) defines; the server
  opens the one channel that [Server Channels](/endpoints/containers-tools-connect/server/)
  defines.
...
4. **The server serves the scope.** The server serves every channel
   the connector opens on the run's own connection to the container's
   proxy — its tree, read, write and transfer scopes, and its MCP exchanges as
   channels on the run's begin scope — as its section defines,
   ...
   opens leaves out every FUSE mount of the run and no other mount,
   as the runner's does.
...
- **The container's own asks go to the runner.** Nothing the proxy
  asks on the begin scope, and nothing a mount asks, reaches a
  connector. The runner's scope answers them.
