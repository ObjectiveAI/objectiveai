...
summary: 'Tag 5: the client sends the params of the MCP method; the server opens the proxy’s /tool/list-tools and relays exactly one channel response, the result or the MCP error, and the channel response finish.'
order: 5
draft: false
---

The client opens this channel for the tools the MCP server of the container offers. The server opens
[`/tool/list-tools`](/proxy/tool/list-tools/) on the proxy with the params
