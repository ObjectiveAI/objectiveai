---
title: 'version'
summary: 'Tag 11: a client asks which revision of this specification a server implements; the server sends exactly one response carrying the string 2.3.0, and the scope finishes.'
order: 11
draft: false
---

A client opens the scope with a request whose payload is the tag byte
`11` and nothing more. The server sends exactly one response, whose
