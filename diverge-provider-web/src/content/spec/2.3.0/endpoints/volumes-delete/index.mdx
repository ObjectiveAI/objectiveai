...
summary: 'Tag 13: a client names a volume from its listing; the server sends exactly one response stating that the volume no longer exists, that it is mounted and was not deleted, or an error, and the scope finishes; the deletion is not reversible.'
order: 13
draft: false
---

A client opens the scope with a request whose payload is the tag byte
`13` followed by the name of a volume. The server sends exactly one
...
- **A volume mounted.** When, at the time of the request, the volume
  is mounted in any running container or served on any
  [volumes::serve](/endpoints/volumes-serve/) scope — a volume may be mounted or served in several at once — or
  is under a stat, an edit or another delete,
