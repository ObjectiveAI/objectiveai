#!/usr/bin/env node
// Generates the cli/command module's typed `execute` functions from
// the Rust SDK's CLI command tree.
//
...
//     never pass it. The executor serializes the JSON and hands it to
//     the cli via its top-level `--request` flag — Request→argv never
//     exists in JS.
//
...
function parseBody(body, context) {
  ...
  if (!tail) {
    throw new Error(
      `${context}: unrecognized body residue \`${trimmed}\` — extend the statement grammar in install-command-execute.cjs`,
    );
  }
...
function main() {
  if (!fs.existsSync(RUST_COMMAND_DIR)) {
    console.error(
      `install-command-execute: rust command dir missing: ${RUST_COMMAND_DIR}`,
    );
    process.exit(2);
  }
  ...
  const errorTitle = (() => {
    ...
    if (!m) {
      console.error(
        "install-command-execute: could not find schemars rename for crate::cli::Error",
      );
      process.exit(2);
    }
    return m[1];
  ...
  if (!errorModule) {
    console.error(
      `install-command-execute: zod module for \`${errorTitle}\` missing — run install-zod.cjs first`,
    );
    process.exit(2);
  }
  ...
  if (skipped.length > 0) {
    console.warn(
      `install-command-execute: SKIPPED ${skipped.length} scope(s)/fn(s):`,
    );
