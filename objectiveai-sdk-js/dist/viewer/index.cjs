...
async function _unlisten2(event, eventId) {
...
async function listen2(event, handler, options) {
  ...
  return invoke("plugin:event|listen", {
    ...
    return async () => _unlisten2(event, eventId);
  });
}
async function once(event, handler, options) {
  return listen2(event, (eventData) => {
    void _unlisten2(event, eventData.id);
    handler(eventData);
  }, options);
}
...
exports.ResponseItemStream = ResponseItemStream;
