...
export const McpServerSchema = z.object({
  ...
  laboratory_id: z.string().nullable().describe("The laboratory's ASSISTANT-FACING composite id -\n`{machineID}/{base62(state)}/{base62(laboratoryID)}` - what\n`laboratory_transfer` takes as `source`/`destination`. Present\nexactly when `laboratory` is (and its marker carries the machine\npair).").meta({ omitempty: true }).optional(),
