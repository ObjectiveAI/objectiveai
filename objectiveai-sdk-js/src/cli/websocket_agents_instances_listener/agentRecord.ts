...
import { z } from "zod";
import { CliWebsocketAgentsInstancesListenerAttachedLaboratorySchema } from "./attachedLaboratory";
...
export const CliWebsocketAgentsInstancesListenerAgentRecordSchema = z.object({
  ...
  attached_laboratories: z.array(CliWebsocketAgentsInstancesListenerAttachedLaboratorySchema).default([]).describe("The agent's ATTACHED laboratories — the effective set the next\nspawn pass dials (the AIH's own attachments UNION its bound\ntags'), oldest-attached first. Live-tracked: attach/detach may\nhappen at any time, active agents included."),
