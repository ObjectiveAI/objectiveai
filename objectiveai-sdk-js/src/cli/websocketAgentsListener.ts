 ...
 * `cli::websocket_agents_listener::WebSocketAgentsListener`, identical in
 ...
 * - {@link WebSocketAgentsListener.agents} — snapshot of the current set
 *   (sorted by AIH).
 * - an on-change **callback** ({@link WebSocketAgentsListenerOptions.onChange}),
 *   invoked with the full refreshed set on every applied change.
 * - {@link WebSocketAgentsListener.subscribe} — a Promise that resolves on
 *   the next change.
 *
 ...
 * {@link WebSocketAgentsListener.subscribe} resolves. Reconnection (the
...
import type {
  CliWebsocketAgentsListenerAgentEvent,
  CliWebsocketAgentsListenerAgentRecord,
} from "./websocket_agents_listener";

type AgentRecord = CliWebsocketAgentsListenerAgentRecord;
type AgentEvent = CliWebsocketAgentsListenerAgentEvent;

export interface WebSocketAgentsListenerOptions {
   ...
   * for the full state on demand use {@link WebSocketAgentsListener.agents}. */
  onChange?: (agents: AgentRecord[]) => void;
}

/**
 * The materialized `/agents/instances/list` view — construct via
 * {@link WebSocketAgentsListener.connect}.
 *
 * ```ts
 * const listener = await WebSocketAgentsListener.connect(
 ...
 * ```
 */
export class WebSocketAgentsListener {
  #ws: WebSocket;
  #closed = false;
  ...
  static connect(
    url: string,
    options?: WebSocketAgentsListenerOptions,
  ): Promise<WebSocketAgentsListener> {
    return new Promise((resolve, reject) => {
      ...
      ws.onopen = () => {
        ...
        resolve(new WebSocketAgentsListener(ws, options?.onChange));
      };
   ...
   * read, or use {@link WebSocketAgentsListenerOptions.onChange} for
