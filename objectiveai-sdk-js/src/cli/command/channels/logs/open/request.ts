...
import { z } from "zod";
import { CliCommandChannelsLogsOpenPathSchema } from "./path";

export const CliCommandChannelsLogsOpenRequestSchema = z.object({
  channel_id: z.string(),
  entry_id: z.number().int().min(-9223372036854776000).max(9223372036854776000).describe("The `channel_messages.id` to open."),
  ...
  path_type: CliCommandChannelsLogsOpenPathSchema,
...
}).meta({ title: "cli.command.channels.logs.open.Request" });
export type CliCommandChannelsLogsOpenRequest = z.infer<typeof CliCommandChannelsLogsOpenRequestSchema>;
