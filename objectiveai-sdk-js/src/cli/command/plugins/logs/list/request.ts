...
export const CliCommandPluginsLogsListRequestSchema = z.object({
  after_id: z.number().int().min(-9223372036854776000).max(9223372036854776000).nullable().describe("Skip rows with `\"index\" <= after_id`. Use the highest `index`\nfrom a previous page to paginate forward.").meta({ omitempty: true }).optional(),
  ...
  limit: z.number().int().min(-9223372036854776000).max(9223372036854776000).nullable().describe("Cap on rows returned. `None` = unlimited.").meta({ omitempty: true }).optional(),
  ...
  version: z.string(),
}).meta({ title: "cli.command.plugins.logs.list.Request" });
export type CliCommandPluginsLogsListRequest = z.infer<typeof CliCommandPluginsLogsListRequestSchema>;
