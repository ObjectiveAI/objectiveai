...
import { z } from "zod";
import { CliCommandApiConfigMcpBackoffSetPathSchema } from "./path";
import { CliCommandSetScopeSchema } from "../../../../setScope";

export const CliCommandApiConfigMcpBackoffSetRequestSchema = z.object({
  ...
  path_type: CliCommandApiConfigMcpBackoffSetPathSchema,
  ...
  value: z.string().describe("The new backoff, as a single JSON blob (an `mcp.Backoff` object).\nCarried verbatim as a string here; the cli handler parses it."),
}).meta({ title: "cli.command.api.config.mcp_backoff.set.Request" });
export type CliCommandApiConfigMcpBackoffSetRequest = z.infer<typeof CliCommandApiConfigMcpBackoffSetRequestSchema>;
