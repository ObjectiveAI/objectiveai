...
import { z } from "zod";
import { CliCommandLaboratoriesConnectPathSchema } from "./path";

export const CliCommandLaboratoriesConnectRequestSchema = z.object({
  address: z.string().nullable().describe("The daemon `ws://` base address to connect to. `None` = the\nLOCAL daemon (ensured + resolved by the CLI).").meta({ omitempty: true }).optional(),
  id: z.string().describe("The laboratory id (must already be created)."),
  ...
  path_type: CliCommandLaboratoriesConnectPathSchema,
...
}).meta({ title: "cli.command.laboratories.connect.Request" });
export type CliCommandLaboratoriesConnectRequest = z.infer<typeof CliCommandLaboratoriesConnectRequestSchema>;
