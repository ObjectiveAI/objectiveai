...
import { z } from "zod";
import { CliCommandDevelopmentViewerSetPathSchema } from "./path";

export const CliCommandDevelopmentViewerSetRequestSchema = z.object({
  ...
  path: z.string().describe("ABSOLUTE path to the `objectiveai-viewer` directory of a source\ncheckout (the one holding `tauri.conf.json`; pnpm resolves the\nworkspace upward from it)."),
  path_type: CliCommandDevelopmentViewerSetPathSchema,
...
}).meta({ title: "cli.command.development.viewer.set.Request" });
export type CliCommandDevelopmentViewerSetRequest = z.infer<typeof CliCommandDevelopmentViewerSetRequestSchema>;
