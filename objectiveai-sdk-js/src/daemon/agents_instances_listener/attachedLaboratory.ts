...
export const DaemonAgentsInstancesListenerAttachedLaboratorySchema = z.object({
...
}).describe("One laboratory attachment on the agent, as carried by\n[`AgentRecord::attached_laboratories`]. Mirrors `agents instances\nget`'s attachment shape (listener-local — this wire is a mirror,\nnot a re-export).").meta({ title: "daemon.agents_instances_listener.AttachedLaboratory" });
export type DaemonAgentsInstancesListenerAttachedLaboratory = z.infer<typeof DaemonAgentsInstancesListenerAttachedLaboratorySchema>;
