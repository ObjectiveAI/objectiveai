...
/**
 * Behavior tests for the `CommandListener` `/listen` SSE consumer:
...
import { CommandListener, ResponseItemStream } from "./commandListener";
...
}): Promise<{ listener: CommandListener; sse: MockSseConnection }> {
  const listener = await new Client(
    "http://127.0.0.1:1",
    options,
  ).commandListener();
  const sse =
...
describe("CommandListener", () => {
  ...
  it("rejects connect on a non-2xx response (refused signature)", async () => {
    stubFetch(401);
    await expect(
      new Client("http://127.0.0.1:1").commandListener(),
    ).rejects.toThrow(/401/);
  });
  ...
  it("yields an envelope per request frame and feeds its stream by id", async () => {
    ...
    expect(
      (run.identity as { agent_id?: string | null }).agent_id,
    ).toBe("agent-a");
