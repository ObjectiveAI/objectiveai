#!/usr/bin/env bash
# Computes a SHA256 fingerprint of all source files that affect the MCP build.
#
# Usage:
...
compute_fingerprint() {
  {
    ...
    find "$REPO_ROOT/objectiveai-cli/src" -type f -name '*.rs' | sort
    echo "$REPO_ROOT/objectiveai-cli/Cargo.toml"
