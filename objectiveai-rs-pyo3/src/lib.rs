...
fn prompt_id(prompt: &Bound<'_, PyAny>) -> PyResult<String> {
    let mut prompt: Vec<objectiveai::agent::completions::message::Message> = from_py(prompt)?;
    objectiveai::agent::completions::message::prompt::prepare(&mut prompt);
    Ok(objectiveai::agent::completions::message::prompt::id(&prompt))
}
...
fn vector_response_id(response: &Bound<'_, PyAny>) -> PyResult<String> {
    let mut response: objectiveai::agent::completions::message::RichContent = from_py(response)?;
    response.prepare();
    Ok(response.id())
}
...
fn validate_function_input(
    ...
    let function: objectiveai::functions::Function = from_py(function)?;
    let input: objectiveai::functions::expression::InputValue = from_py(input)?;
...
fn compile_function_input_merge(
    ...
    let function: objectiveai::functions::Function = from_py(function)?;
    let input: Vec<objectiveai::functions::expression::InputValue> = from_py(input)?;
    let input_merge = function
        .compile_input_merge(&objectiveai::functions::expression::InputValue::Array(input))
        .map_err(|e| PyValueError::new_err(e.to_string()))?;
    match input_merge {
...
fn check_vector_fields(fields: &Bound<'_, PyAny>) -> PyResult<()> {
    let fields: objectiveai::functions::check::VectorFieldsValidation = from_py(fields)?;
...
fn check_scalar_fields(fields: &Bound<'_, PyAny>) -> PyResult<()> {
    let fields: objectiveai::functions::check::ScalarFieldsValidation = from_py(fields)?;
...
fn alpha_check_branch_scalar_function(
    ...
    objectiveai::functions::alpha_scalar::check::check_alpha_branch_scalar_function(
        &function,
        children.as_ref(),
        None,
    )
...
fn alpha_check_branch_vector_function(
    ...
    objectiveai::functions::alpha_vector::check::check_alpha_branch_vector_function(
        &function,
        children.as_ref(),
        None,
    )
