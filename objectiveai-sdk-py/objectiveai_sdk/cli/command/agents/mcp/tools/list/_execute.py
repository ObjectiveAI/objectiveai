...
from objectiveai_sdk.cli.command.agents.mcp.tools.list.request import Request
...
_execute_adapter = TypeAdapter(
    Annotated[Union[CliError, ListToolsResult], Field(union_mode="left_to_right")]
)


async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, ListToolsResult]:
    ...
    if first is None:
        raise RuntimeError("agents mcp tools list: cli produced no output before the end marker")
    return first
