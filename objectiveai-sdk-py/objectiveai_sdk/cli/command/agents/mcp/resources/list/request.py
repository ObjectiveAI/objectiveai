...
from objectiveai_sdk.cli.command.agents.mcp.resources.list.path import Path
from objectiveai_sdk.mcp.resource.list_resources_request import ListResourcesRequest


class Request(BaseModel):
    model_config = ConfigDict(title='cli.command.agents.mcp.resources.list.Request')
    ...
    name: Optional[str] = Field(None, description='Restrict the listing to the single server with this name (the\nrouting prefix `agents mcp servers list` reports). `None` lists\nevery server.', json_schema_extra={'omitempty': True})
