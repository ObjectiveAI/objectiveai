...
from objectiveai_sdk.cli.command.agents.logs.token_usage.subscribe.path import Path


class Request(BaseModel):
    model_config = ConfigDict(title='cli.command.agents.logs.token_usage.subscribe.Request')

    agent_instance_hierarchy: str = Field(..., description='The full agent instance hierarchy to watch.')
    ...
    path_type: Path
    previous: Optional[Annotated[int, Field(ge=-9223372036854775808, le=9223372036854775807)]] = Field(None, description='The last-seen `total_tokens`. When set and the stored value\nalready differs, the command returns immediately instead of\nsubscribing. `None` on the wire = always subscribe.', json_schema_extra={'omitempty': True})
