...
from objectiveai_sdk.cli.command.agents.logs.list.path import Path
from objectiveai_sdk.cli.command.agents.logs.list.target import Target


class Request(BaseModel):
    model_config = ConfigDict(title='cli.command.agents.logs.list.Request')
    ...
    path_type: Path
    pending: bool = Field(..., description='`true` = list only pending (unfinalized) rows; `false` = list\nevery logged row. Selected on the CLI by `--pending` / `--all`.')
