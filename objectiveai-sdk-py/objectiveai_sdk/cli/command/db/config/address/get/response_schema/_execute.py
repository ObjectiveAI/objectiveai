...
from objectiveai_sdk.cli.command.db.config.address.get.response_schema.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, JsonValue]:
    ...
    if first is None:
        raise RuntimeError("db config address get response_schema: cli produced no output before the end marker")
    return first
