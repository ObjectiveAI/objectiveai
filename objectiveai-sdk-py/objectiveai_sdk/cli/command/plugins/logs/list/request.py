...
from objectiveai_sdk.cli.command.plugins.logs.list.path import Path


class Request(BaseModel):
    model_config = ConfigDict(title='cli.command.plugins.logs.list.Request')

    after_id: Optional[Annotated[int, Field(ge=-9223372036854775808, le=9223372036854775807)]] = Field(None, description='Skip rows with `"index" <= after_id`. Use the highest `index`\nfrom a previous page to paginate forward.', json_schema_extra={'omitempty': True})
    ...
    limit: Optional[Annotated[int, Field(ge=-9223372036854775808, le=9223372036854775807)]] = Field(None, description='Cap on rows returned. `None` = unlimited.', json_schema_extra={'omitempty': True})
