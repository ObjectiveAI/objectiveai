...
from objectiveai_sdk.cli.command.channels.logs.list.path import Path


class Request(BaseModel):
    model_config = ConfigDict(title='cli.command.channels.logs.list.Request')

    after_id: Optional[Annotated[int, Field(ge=-9223372036854775808, le=9223372036854775807)]] = Field(None, description='Skip entries with `id <= after_id`.', json_schema_extra={'omitempty': True})
    channel_id: str
    ...
    limit: Optional[Annotated[int, Field(ge=-9223372036854775808, le=9223372036854775807)]] = Field(None, description='Cap on entries returned.', json_schema_extra={'omitempty': True})
    ...
    path_type: Path
    pending: bool = Field(..., description='`true` = `--pending` (unread for this role, advances the\nwatermark); `false` = `--all`.')
