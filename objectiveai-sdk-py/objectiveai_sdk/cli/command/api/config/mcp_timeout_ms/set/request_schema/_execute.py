...
from objectiveai_sdk.cli.command.api.config.mcp_timeout_ms.set.request_schema.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, JsonValue]:
    ...
    if first is None:
        raise RuntimeError("api config mcp_timeout_ms set request_schema: cli produced no output before the end marker")
    return first
