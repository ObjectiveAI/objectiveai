...
from objectiveai_sdk.cli.command.api.config.mcp_call_timeout_ms.set.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, Ok]:
    ...
    if first is None:
        raise RuntimeError("api config mcp_call_timeout_ms set: cli produced no output before the end marker")
    return first
