...
from objectiveai_sdk.cli.command.api.config.mcp_authorization.del_.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, Ok]:
    ...
    if first is None:
        raise RuntimeError("api config mcp_authorization del: cli produced no output before the end marker")
    return first
