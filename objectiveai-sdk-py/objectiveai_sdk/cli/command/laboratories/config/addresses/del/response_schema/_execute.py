...
from objectiveai_sdk.cli.command.laboratories.config.addresses.del_.response_schema.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, JsonValue]:
    ...
    if first is None:
        raise RuntimeError("laboratories config addresses del response_schema: cli produced no output before the end marker")
    return first
