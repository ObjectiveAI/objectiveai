...
from objectiveai_sdk.cli.command.laboratories.config.addresses.del_.request import Request
...
async def execute(executor: CommandExecutor, request: Request) -> Union[CliError, Ok]:
    ...
    if first is None:
        raise RuntimeError("laboratories config addresses del: cli produced no output before the end marker")
    return first
