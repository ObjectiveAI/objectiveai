...
/// Asserts that every folder in assets/mock/functions/inventions/ contains a
/// parameters.json, and that the `name` field in parameters.json matches the
/// folder name.
#[test]
fn mock_invention_state_folder_names_match_params() {
    let inventions_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("assets/mock/functions/inventions");
    ...
    for entry in std::fs::read_dir(&inventions_dir).unwrap() {
        let entry = entry.unwrap();
        let path = entry.path();
        if !path.is_dir() {
            continue;
        ...
        if !params_path.exists() {
            violations.push(format!("{}: missing parameters.json", folder_name));
            continue;
        }

        let content = std::fs::read_to_string(&params_path).unwrap();
        let value: serde_json::Value = serde_json::from_str(&content)
            .unwrap_or_else(|e| panic!("{}/parameters.json is invalid JSON: {e}", folder_name));

        let name = value.get("name").and_then(|n| n.as_str()).unwrap_or("");
        if name != folder_name {
            violations.push(format!(
                "{}: parameters.json name is {:?}, expected {:?}",
    ...
    assert!(
        violations.is_empty(),
        "Mock invention state folder/name mismatches:\n  {}",
        violations.join("\n  "),
    );
}
