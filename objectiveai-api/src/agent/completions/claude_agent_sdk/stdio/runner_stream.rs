...
///
/// Drop does **not** cancel the in-flight SDK work. The Claude Agent
/// SDK doesn't expose a contract for stopping between billing events,
/// so once a request is sent the runner runs it to natural
/// completion. A consumer that walks away mid-stream is still billed
/// for the work in flight; the wrapper-side `is_cancelled()` check
/// only bounds *future* tool-call iterations from being started.
