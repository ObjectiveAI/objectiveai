//! Per-`(response_id, McpKind)` broadcast registry feeding the
...
/// Maps `(response_id, McpKind)` to a `broadcast::Sender` the
/// conduit publishes to and the per-MCP GET-SSE handler subscribes
/// from. `response_id` is the per-agent leaf response id (same value
/// stamped on every reverse-channel request as
/// `X-OBJECTIVEAI-RESPONSE-ID`); `McpKind` distinguishes the
/// `/objectiveai` builtin from each `{owner}/{name}/{ver}/{mcp}`
/// plugin upstream.
///
...
impl McpListenerRegistry {
    ...
    /// Subscribe to the broadcast keyed by `(response_id, mcp_kind)`.
    ...
    pub fn subscribe(
        ...
        let key = (response_id.to_string(), mcp_kind.clone());
        let entry = self
            .inner
            .entry(key)
    ...
    /// Publish to the broadcast keyed by `(response_id, mcp_kind)`.
...
mod tests {
    ...
    async fn subscribe_then_publish_round_trips_kind() {
        ...
        let mut rx = reg.subscribe("rid", &McpKind::ObjectiveAi);
        reg.publish("rid", &McpKind::ObjectiveAi, McpListChangedKind::Tools);
    ...
    async fn publish_with_no_subscribers_is_silent_no_op() {
        let reg = McpListenerRegistry::new();
        // No panic, no error.
        reg.publish("rid", &McpKind::ObjectiveAi, McpListChangedKind::Resources);
    }

    #[tokio::test]
    async fn gc_drops_channel_only_when_no_receivers() {
        let reg = McpListenerRegistry::new();
        let rx = reg.subscribe("rid", &McpKind::ObjectiveAi);
        // With one receiver, gc is a no-op.
        reg.gc("rid", &McpKind::ObjectiveAi);
        // Drop the receiver, then gc should remove.
        drop(rx);
        reg.gc("rid", &McpKind::ObjectiveAi);
        // A fresh subscribe should land on a brand-new channel.
        let mut rx2 = reg.subscribe("rid", &McpKind::ObjectiveAi);
        reg.publish("rid", &McpKind::ObjectiveAi, McpListChangedKind::Tools);
