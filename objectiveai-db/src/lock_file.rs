// NOTE: this module is a copy of `objectiveai-cli/src/lock_file.rs`
// (which keeps its own copy for agent lock files). objectiveai-db
// only needs `try_acquire` + `wait_release` for the bootstrap mutex,
// but the file is copied whole so the two stay diff-able; unused
// helpers are tolerated via `allow(dead_code)`. If you fix a bug
