...
`<OBJECTIVEAI_DIR>/state/<OBJECTIVEAI_STATE>/db/` cluster, bound to a
fixed `ADDRESS`/`PORT`,
...
| var | default | meaning |
|---|---|---|
| `OBJECTIVEAI_DIR` | `~/.objectiveai` | layout root (postgres install at `bin/pg-bin/`, shared by every state) |
| `OBJECTIVEAI_STATE` | `default` | state name (`[A-Za-z0-9_-]+`); cluster at `state/<state>/db/` |
