//! `/tool/list-tools`: what tools the container's server offers.
...
/// `/tool/list-tools`. Accepted as many times as the server opens it,
/// each one exchange.
pub async fn list_tools(State(tool): State<Arc<Tool>>, upgrade: WebSocketUpgrade) -> Response {
    upgrade
