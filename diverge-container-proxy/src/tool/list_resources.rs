//! `/tool/list-resources`: what resources the container's server offers.
...
/// `/tool/list-resources`. Accepted as many times as the server opens it,
/// each one exchange.
pub async fn list_resources(State(tool): State<Arc<Tool>>, upgrade: WebSocketUpgrade) -> Response {
    upgrade
