...
use diverge_container_proxy_sdk::agent::run::request::Request;
...
/// Start one loop on `content`, on a task of its own: what comes back
...
/// never waits on it.
pub fn start(proxy: Arc<Proxy>, content: Vec<ContentBlock>) -> oneshot::Receiver<Result<reqwest::Response, Error>> {
    let (sender, receiver) = oneshot::channel();
    tokio::spawn(async move {
        let body = match serde_json::to_vec(&Request { content }) {
            Ok(body) => body,
            Err(error) => {
                let _ = sender.send(Err(Error(serde_json::json!({
                    "kind": "agent",
                    "error": format!("the message did not serialize: {error}"),
                }))));
                return;
            }
        };
