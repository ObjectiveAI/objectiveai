...
/// [`diverge_provider_sdk::endpoints::volumes::read::client::request::Frame`] — and mean the same here,
...
pub struct Frame(pub diverge_provider_sdk::endpoints::volumes::read::client::request::Frame);
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("volume read request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected volume read request tag {TAG}, found {tag}")
            }
