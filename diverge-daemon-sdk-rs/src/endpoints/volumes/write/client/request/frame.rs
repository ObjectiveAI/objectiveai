...
/// [`diverge_provider_sdk::endpoints::volumes::write::client::request::Frame`] — and mean the same here,
...
pub struct Frame(pub diverge_provider_sdk::endpoints::volumes::write::client::request::Frame);
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("volume write request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected volume write request tag {TAG}, found {tag}")
            }
