...
/// [`diverge_provider_sdk::endpoints::volumes::edit::client::request::Frame`] — and mean the same here,
...
pub struct Frame(pub diverge_provider_sdk::endpoints::volumes::edit::client::request::Frame);
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("volume edit request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected volume edit request tag {TAG}, found {tag}")
            }
