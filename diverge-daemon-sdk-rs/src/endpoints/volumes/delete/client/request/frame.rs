//! What a client's request frame carries for a volume delete.
...
/// [`diverge_provider_sdk::endpoints::volumes::delete::client::request::Frame`] — and mean the same here,
...
pub struct Frame(pub diverge_provider_sdk::endpoints::volumes::delete::client::request::Frame);
...
/// A volume delete request that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("volume delete request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected volume delete request tag {TAG}, found {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "volume delete request did not parse: {error}")
            }
        }
    }
}
