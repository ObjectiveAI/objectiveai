...
/// provider's [`diverge_provider_sdk::endpoints::volumes::create_capacity::client::request::Frame`] is, the question
/// having no parameter.
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("volume create capacity request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected volume create capacity request tag {TAG}, found {tag}")
            }
        }
    }
}
