...
/// Ask the daemon for every agent of the caller's.
///
...
/// tag as every request of the agents family is, and so that a
/// member has somewhere to land the day one is wanted.
...
/// A list request frame that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("agents list request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected agents list request tag {TAG}, found {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "agents list request did not parse: {error}")
            }
        }
    }
}
