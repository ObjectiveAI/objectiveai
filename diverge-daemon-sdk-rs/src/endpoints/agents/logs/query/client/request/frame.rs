//! What a client's request frame carries for a query.
...
/// A query request frame that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl std::fmt::Display for FrameError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FrameError::Empty => f.write_str("agents logs query request frame is empty"),
            FrameError::UnexpectedTag(tag) => {
                write!(f, "expected agents logs query request tag {TAG}, found {tag}")
            }
            FrameError::Body(error) => {
                write!(f, "agents logs query request did not parse: {error}")
            }
        }
    }
}
