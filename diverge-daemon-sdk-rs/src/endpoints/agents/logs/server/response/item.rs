...
/// log's, so an item reads as the chunk it is with `logs_id` and
/// `created` added; or an error, whole, under `error`. Whose message
/// a user part is, its own `key` says.
///
/// `logs_id` rather than `id`, because a tool call and a tool
...
pub struct Item {
    ...
    /// from `1`, and never repeats. What the request's `logs_id_from`
    /// and `logs_id_to` span.
    pub logs_id: u64,
