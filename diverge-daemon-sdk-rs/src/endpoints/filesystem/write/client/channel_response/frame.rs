...
use diverge_provider_sdk::shared::containers::write_bytes;
...
/// abandoned: the daemon discards what it wrote, and answers the
/// scope with an error of its own.
///
/// # The failure is this scope's, the bytes are not
///
/// [`Body`](Self::Body) carries
/// [`write_bytes::response::Frame`](diverge_provider_sdk::shared::containers::write_bytes::response::Frame),
/// which is what a piece of a file is anywhere — each at most
/// [`CHUNK_SIZE`](diverge_provider_sdk::CHUNK_SIZE), appended to the
/// pieces before it. The [`Error`](Self::Error) beside it is the
/// exchange's own.
...
/// A filesystem write content frame that could not be read.
#[derive(Debug)]
pub enum FrameError {
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => f.write_str("filesystem write content frame is empty"),
            FrameError::UnknownTag(tag) => {
                write!(f, "unknown filesystem write content frame tag {tag}")
            }
            FrameError::Error(error) => {
                write!(f, "filesystem write content error did not parse: {error}")
            }
        }
    }
}
