//! What a client's channel request frame carries on a filetree scope.
...
/// What a client asks the daemon for while a filetree is open.
///
...
pub enum Frame {
    ...
    /// watch, since the channel is on its scope — and nothing comes
    /// back on this channel: the daemon sends no channel response and
    /// no channel response finish on it. What comes back is the
    /// scope's finish: the daemon drops the watch and finishes the
    /// scope, and frames it had already sent precede the finish,
    /// since a cancel is a request and not a recall. This is how a
    /// watch, which never ends on its own, is ended. A cancel of a
    /// scope that has already finished, or is finishing, changes
    /// nothing; a second cancel changes nothing either.
    Cancel,
}
...
/// A filetree channel request that could not be read.
///
...
impl fmt::Display for FrameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FrameError::Empty => f.write_str("filesystem filetree channel request frame is empty"),
            FrameError::UnknownTag(tag) => {
                write!(f, "unknown filesystem filetree channel request tag {tag}")
            }
        }
    }
}
