...
def test_push_fuzz(stream):
    seed = stream * 1000
    init = objectiveai_pyo3.generate_function_invention_recursive_chunk(seed)
    py_acc = FunctionInventionRecursiveChunk.model_validate(init)
    pyo3_acc = copy.deepcopy(init)
    seed += 1

    for j in range(20):
        chunk = objectiveai_pyo3.generate_function_invention_recursive_chunk(seed)
        seed += 1

        py_acc.push(FunctionInventionRecursiveChunk.model_validate(chunk))
        pyo3_acc = objectiveai_pyo3.function_invention_recursive_chunk_merged(pyo3_acc, chunk)
