//! The `python` agent, as a container.
//!
...
//! `GET /schema` is the JSON Schema of the agent value this image
//! accepts; `POST /register` is that value, told once for the
//! container's life before any loop — a run before it is refused —
//! and is when its source is written to disk and its requirements
//! installed. No model: every turn RUNS the agent's Python against
//! the whole conversation and the caller's tool and resource lists,
//! and reads what its last expression answers. Its tool calls go
//! through the proxy's MCP server; the history it resumes from, and
//! leaves behind, is one row in the caller's database, reached
//! through the proxy's loopback pgwire.
//!
...
//! proxy — the database, the MCP session — is `POST /run` itself.
//! Registration touches the disk and pip, never the proxy. Making
//! the [`Client`] is no I/O.
//!
...
//! calling a tool that does not exist — and the first item of the
...
use diverge_provider_sdk::container_proxy::agent::run as run_endpoint;
...
/// notification.
///
/// A run before registration is refused (`409`) before anything
/// else. One run at a time: the [`Claim`] is taken next and refused
/// with `409` while another holds it, and it lives exactly as long
/// as the stream — released on every refusal below, and otherwise
/// captured into the stream, so it drops with it, finished or
/// abandoned. The queue is opened for this run right behind the
/// claim; every refusal before the loop exists closes it again, so a
/// message waiting for a loop that will never look is missed
/// honestly. Once the loop exists its own guard closes the queue
/// however the run ends. The history comes from the cache when this
/// container has loaded or saved one, and from the row only the
/// first time.
async fn run(
    State(client): State<Arc<Client>>,
    Json(request): Json<run_endpoint::request::Request>,
