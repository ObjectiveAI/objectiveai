//! Running a laboratory from an image.
//!
...
//! pull on channels the provider opens, then the container's id and
//! its filesystem on channel `0` for as long as it runs — so a caller
//! does not start one and then separately ask to watch it.
//!
