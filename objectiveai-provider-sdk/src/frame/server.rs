...
///
/// [`Request`](Self::Request) is the only variant that names a
/// channel, because it is the only one that opens one.
/// [`Ack`](Self::Ack), [`Body`](Self::Body) and
/// [`Finish`](Self::Finish) carry none: they answer the client's own
/// request, which is always channel `0`.
