...
type CliCommandUserRequestResponseSchemaRequest struct {
	...
	PathType CliCommandUserRequestResponseSchemaPath `json:"path_type"`
...
func (CliCommandUserRequestResponseSchemaRequest) SchemaTitle() string { return "cli.command.user.request.response_schema.Request" }
func (v CliCommandUserRequestResponseSchemaRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandUserRequestResponseSchemaRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandUserRequestResponseSchemaRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandUserRequestResponseSchemaRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandUserRequestResponseSchemaRequest(alias)
	return nil
}
