...
type DaemonAgentsInstancesListenerAgentRecord struct {
	...
	AttachedLaboratories []DaemonAgentsInstancesListenerAttachedLaboratory `json:"attached_laboratories" default:"[]"`
...
func (DaemonAgentsInstancesListenerAgentRecord) SchemaTitle() string { return "daemon.agents_instances_listener.AgentRecord" }
func (v DaemonAgentsInstancesListenerAgentRecord) Validate() error {
	return variantValidator.Struct(v)
}

func (v *DaemonAgentsInstancesListenerAgentRecord) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"active", "active_laboratories", "agent_instance_hierarchy", "attached_laboratories", "logged", "queued", "tags"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("DaemonAgentsInstancesListenerAgentRecord: missing required field %q", key)
		}
	}
	type Alias DaemonAgentsInstancesListenerAgentRecord
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = DaemonAgentsInstancesListenerAgentRecord(alias)
	return nil
}
