...
type CliCommandLaboratoriesDeleteRequest struct {
	ID string `json:"id"`
	// jq filter applied to the JSON output. Ignored when `python`
	// is also set — python overrides jq.
	Jq *string `json:"jq"`
	Kind CliCommandLaboratoriesDeleteKind `json:"kind"`
	...
	PathType CliCommandLaboratoriesDeletePath `json:"path_type"`
...
func (CliCommandLaboratoriesDeleteRequest) SchemaTitle() string { return "cli.command.laboratories.delete.Request" }
func (v CliCommandLaboratoriesDeleteRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandLaboratoriesDeleteRequest) UnmarshalJSON(data []byte) error {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}
	for _, key := range []string{"id", "kind", "path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandLaboratoriesDeleteRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandLaboratoriesDeleteRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandLaboratoriesDeleteRequest(alias)
	return nil
}
