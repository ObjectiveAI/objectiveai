...
type CliCommandApiConfigMcpTimeoutMsGetRequest struct {
...
func (CliCommandApiConfigMcpTimeoutMsGetRequest) SchemaTitle() string { return "cli.command.api.config.mcp_timeout_ms.get.Request" }
func (v CliCommandApiConfigMcpTimeoutMsGetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandApiConfigMcpTimeoutMsGetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "scope"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandApiConfigMcpTimeoutMsGetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandApiConfigMcpTimeoutMsGetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandApiConfigMcpTimeoutMsGetRequest(alias)
	return nil
}
