...
func (v *FunctionsInventionsStateParams) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"depth", "max_branch_width", "max_leaf_width", "min_branch_width", "min_leaf_width", "name", "spec"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("FunctionsInventionsStateParams: missing required field %q", key)
		}
	}
	type Alias FunctionsInventionsStateParams
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = FunctionsInventionsStateParams(alias)
	return nil
}
