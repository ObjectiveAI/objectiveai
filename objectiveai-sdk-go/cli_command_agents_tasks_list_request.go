...
type CliCommandAgentsTasksListRequest struct {
	...
	PathType CliCommandAgentsTasksListPath `json:"path_type"`
...
func (CliCommandAgentsTasksListRequest) SchemaTitle() string { return "cli.command.agents.tasks.list.Request" }
func (v CliCommandAgentsTasksListRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsTasksListRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"exhausted", "interval", "oneshot", "path_type", "pending"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsTasksListRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsTasksListRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsTasksListRequest(alias)
	return nil
}
