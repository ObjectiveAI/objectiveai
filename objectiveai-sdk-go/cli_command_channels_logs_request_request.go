...
type CliCommandChannelsLogsRequestRequest struct {
	...
	PathType CliCommandChannelsLogsRequestPath `json:"path_type"`
...
func (CliCommandChannelsLogsRequestRequest) SchemaTitle() string { return "cli.command.channels.logs.request.Request" }
func (v CliCommandChannelsLogsRequestRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandChannelsLogsRequestRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"channel_id", "content", "path_type", "secret"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandChannelsLogsRequestRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandChannelsLogsRequestRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandChannelsLogsRequestRequest(alias)
	return nil
}
