...
type CliCommandDaemonKillRequest struct {
...
func (CliCommandDaemonKillRequest) SchemaTitle() string { return "cli.command.daemon.kill.Request" }
func (v CliCommandDaemonKillRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDaemonKillRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDaemonKillRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDaemonKillRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDaemonKillRequest(alias)
	return nil
}
