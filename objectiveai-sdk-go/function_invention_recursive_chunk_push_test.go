...
func TestFunctionInventionRecursiveChunkPush(t *testing.T) {
	for stream := 0; stream < 20; stream++ {
		t.Run(fmt.Sprintf("stream_%d", stream), func(t *testing.T) {
			seed := int64(stream * 1000)
			goAcc, err := GenerateFunctionInventionRecursiveChunk(true, seed)
			if err != nil {
			...
			for j := 0; j < 20; j++ {
				chunk, err := GenerateFunctionInventionRecursiveChunk(true, seed)
				if err != nil {
				...
				cffiMerged, err := FunctionInventionRecursiveChunkMerged(cffiAcc, *chunk)
				if err != nil {
