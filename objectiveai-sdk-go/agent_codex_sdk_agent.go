...
func (AgentCodexSdkAgent) SchemaTitle() string { return "agent.codex_sdk.Agent" }
...
func (v *AgentCodexSdkAgent) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"id", "model", "output_mode", "plugins", "upstream"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("AgentCodexSdkAgent: missing required field %q", key)
		}
	}
	type Alias AgentCodexSdkAgent
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = AgentCodexSdkAgent(alias)
	return nil
}
