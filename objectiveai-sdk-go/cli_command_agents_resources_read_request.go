...
type CliCommandAgentsResourcesReadRequest struct {
	...
	PathType CliCommandAgentsResourcesReadPath `json:"path_type"`
...
func (CliCommandAgentsResourcesReadRequest) SchemaTitle() string { return "cli.command.agents.resources.read.Request" }
func (v CliCommandAgentsResourcesReadRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsResourcesReadRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"params", "path_type", "response_id"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsResourcesReadRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsResourcesReadRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsResourcesReadRequest(alias)
	return nil
}
