...
type CliCommandApiConfigMcpBackoffGetRequest struct {
	...
	PathType CliCommandApiConfigMcpBackoffGetPath `json:"path_type"`
...
func (CliCommandApiConfigMcpBackoffGetRequest) SchemaTitle() string { return "cli.command.api.config.mcp_backoff.get.Request" }
func (v CliCommandApiConfigMcpBackoffGetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandApiConfigMcpBackoffGetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "scope"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandApiConfigMcpBackoffGetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandApiConfigMcpBackoffGetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandApiConfigMcpBackoffGetRequest(alias)
	return nil
}
