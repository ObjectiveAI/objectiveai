...
func (CliCommandAgentsInstancesListRequest) SchemaTitle() string { return "cli.command.agents.instances.list.Request" }
func (v CliCommandAgentsInstancesListRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsInstancesListRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "targets"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsInstancesListRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsInstancesListRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsInstancesListRequest(alias)
	return nil
}
