...
type CliCommandLaboratoriesListRequest struct {
...
func (CliCommandLaboratoriesListRequest) SchemaTitle() string { return "cli.command.laboratories.list.Request" }
func (v CliCommandLaboratoriesListRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandLaboratoriesListRequest) UnmarshalJSON(data []byte) error {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}
	for _, key := range []string{"kind", "path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandLaboratoriesListRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandLaboratoriesListRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandLaboratoriesListRequest(alias)
	return nil
}
