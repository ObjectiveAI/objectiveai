...
type CliCommandPythonRequest struct {
...
func (CliCommandPythonRequest) SchemaTitle() string { return "cli.command.python.Request" }
func (v CliCommandPythonRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandPythonRequest) UnmarshalJSON(data []byte) error {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}
	for _, key := range []string{"code", "input", "path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandPythonRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandPythonRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandPythonRequest(alias)
	return nil
}
