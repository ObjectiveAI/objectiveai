...
type CliCommandAgentsToolsCallRequest struct {
	...
	Params McpToolCallToolRequestParams `json:"params"`
	PathType CliCommandAgentsToolsCallPath `json:"path_type"`
...
func (CliCommandAgentsToolsCallRequest) SchemaTitle() string { return "cli.command.agents.tools.call.Request" }
func (v CliCommandAgentsToolsCallRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsToolsCallRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"params", "path_type", "response_id"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsToolsCallRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsToolsCallRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsToolsCallRequest(alias)
	return nil
}
