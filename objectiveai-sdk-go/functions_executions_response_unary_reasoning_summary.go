...
func (v *FunctionsExecutionsResponseUnaryReasoningSummary) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"agent_full_id", "agent_id", "agent_instance_hierarchy", "created", "id", "messages", "object", "upstream", "usage"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("FunctionsExecutionsResponseUnaryReasoningSummary: missing required field %q", key)
		}
	}
	type Alias FunctionsExecutionsResponseUnaryReasoningSummary
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = FunctionsExecutionsResponseUnaryReasoningSummary(alias)
	return nil
}
