...
func (v *VectorCompletionsResponseUnaryVectorCompletion) UnmarshalJSON(data []byte) error {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}
	for _, key := range []string{"completions", "created", "id", "object", "scores", "swarm", "usage", "votes", "weights"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("VectorCompletionsResponseUnaryVectorCompletion: missing required field %q", key)
		}
	}
	type Alias VectorCompletionsResponseUnaryVectorCompletion
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = VectorCompletionsResponseUnaryVectorCompletion(alias)
	return nil
}
