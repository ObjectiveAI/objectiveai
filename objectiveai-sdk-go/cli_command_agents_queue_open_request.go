...
type CliCommandAgentsQueueOpenRequest struct {
...
func (CliCommandAgentsQueueOpenRequest) SchemaTitle() string { return "cli.command.agents.queue.open.Request" }
func (v CliCommandAgentsQueueOpenRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsQueueOpenRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"id", "path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsQueueOpenRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsQueueOpenRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsQueueOpenRequest(alias)
	return nil
}
