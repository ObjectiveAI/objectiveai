...
// accept POST.
type DaemonChannelListenerChannelOffer struct {
...
func (DaemonChannelListenerChannelOffer) SchemaTitle() string { return "daemon.channel_listener.ChannelOffer" }
func (v DaemonChannelListenerChannelOffer) Validate() error {
	return variantValidator.Struct(v)
}

func (v *DaemonChannelListenerChannelOffer) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"channel_id", "details", "key", "message", "task"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("DaemonChannelListenerChannelOffer: missing required field %q", key)
		}
	}
	type Alias DaemonChannelListenerChannelOffer
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = DaemonChannelListenerChannelOffer(alias)
	return nil
}
