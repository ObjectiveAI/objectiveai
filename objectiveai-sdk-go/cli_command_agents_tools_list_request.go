...
type CliCommandAgentsToolsListRequest struct {
	...
	PathType CliCommandAgentsToolsListPath `json:"path_type"`
...
func (CliCommandAgentsToolsListRequest) SchemaTitle() string { return "cli.command.agents.tools.list.Request" }
func (v CliCommandAgentsToolsListRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsToolsListRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"params", "path_type", "response_id"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsToolsListRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsToolsListRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsToolsListRequest(alias)
	return nil
}
