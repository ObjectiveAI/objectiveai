...
func (McpToolResourceLink) SchemaTitle() string { return "mcp.tool.ResourceLink" }
...
func (v *McpToolResourceLink) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"name", "uri"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("McpToolResourceLink: missing required field %q", key)
		}
	}
	type Alias McpToolResourceLink
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = McpToolResourceLink(alias)
	return nil
}
