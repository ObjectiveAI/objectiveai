...
func (FunctionsInventionsStateAlphaScalarState) SchemaTitle() string { return "functions.inventions.state.AlphaScalarState" }
func (v FunctionsInventionsStateAlphaScalarState) Validate() error {
...
func (v *FunctionsInventionsStateAlphaScalarState) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"depth", "max_branch_width", "max_leaf_width", "min_branch_width", "min_leaf_width", "name", "spec"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("FunctionsInventionsStateAlphaScalarState: missing required field %q", key)
		}
	}
	type Alias FunctionsInventionsStateAlphaScalarState
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = FunctionsInventionsStateAlphaScalarState(alias)
	return nil
}
