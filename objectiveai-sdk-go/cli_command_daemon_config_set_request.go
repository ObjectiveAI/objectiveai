...
type CliCommandDaemonConfigSetRequest struct {
	...
	PathType CliCommandDaemonConfigSetPath `json:"path_type"`
	...
	Value CliCommandDaemonConfigSetValue `json:"value"`
}

func (CliCommandDaemonConfigSetRequest) SchemaTitle() string { return "cli.command.daemon.config.set.Request" }
func (v CliCommandDaemonConfigSetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDaemonConfigSetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "value"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDaemonConfigSetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDaemonConfigSetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDaemonConfigSetRequest(alias)
	return nil
}
