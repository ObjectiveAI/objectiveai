...
func (HttpViewerFunctionExecutionCreateParams) SchemaTitle() string { return "http.viewer.FunctionExecutionCreateParams" }
func (v HttpViewerFunctionExecutionCreateParams) Validate() error {
	return variantValidator.Struct(v)
}

func (v *HttpViewerFunctionExecutionCreateParams) UnmarshalJSON(data []byte) error {
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}
	for _, key := range []string{"function", "id", "input", "profile"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("HttpViewerFunctionExecutionCreateParams: missing required field %q", key)
		}
	}
	type Alias HttpViewerFunctionExecutionCreateParams
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = HttpViewerFunctionExecutionCreateParams(alias)
	return nil
}
