...
type CliCommandAgentsLogsSubscribeRequest struct {
	...
	Kinds *CliCommandAgentsLogsSubscribeKindFilter `json:"kinds,omitempty"`
...
func (CliCommandAgentsLogsSubscribeRequest) SchemaTitle() string { return "cli.command.agents.logs.subscribe.Request" }
func (v CliCommandAgentsLogsSubscribeRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsLogsSubscribeRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "targets"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsLogsSubscribeRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsLogsSubscribeRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsLogsSubscribeRequest(alias)
	return nil
}
