...
type CliCommandAgentsLogsSubscribeKindFilter struct {
...
func (CliCommandAgentsLogsSubscribeKindFilter) SchemaTitle() string { return "cli.command.agents.logs.subscribe.KindFilter" }
func (v CliCommandAgentsLogsSubscribeKindFilter) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsLogsSubscribeKindFilter) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"assistant", "request", "tool"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsLogsSubscribeKindFilter: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsLogsSubscribeKindFilter
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsLogsSubscribeKindFilter(alias)
	return nil
}
