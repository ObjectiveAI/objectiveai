...
type CliCommandLaboratoriesKillRequest struct {
	...
	PathType CliCommandLaboratoriesKillPath `json:"path_type"`
...
func (CliCommandLaboratoriesKillRequest) SchemaTitle() string { return "cli.command.laboratories.kill.Request" }
func (v CliCommandLaboratoriesKillRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandLaboratoriesKillRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "scope"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandLaboratoriesKillRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandLaboratoriesKillRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandLaboratoriesKillRequest(alias)
	return nil
}
