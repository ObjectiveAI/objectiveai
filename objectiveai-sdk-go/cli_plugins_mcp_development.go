...
// image's layout.
type CliPluginsMcpDevelopment struct {
...
func (CliPluginsMcpDevelopment) SchemaTitle() string { return "cli.plugins.McpDevelopment" }
func (v CliPluginsMcpDevelopment) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliPluginsMcpDevelopment) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"caches"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliPluginsMcpDevelopment: missing required field %q", key)
		}
	}
	type Alias CliPluginsMcpDevelopment
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliPluginsMcpDevelopment(alias)
	return nil
}
