...
type CliCommandAgentsQueueListRequest struct {
...
func (CliCommandAgentsQueueListRequest) SchemaTitle() string { return "cli.command.agents.queue.list.Request" }
func (v CliCommandAgentsQueueListRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsQueueListRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "targets"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsQueueListRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsQueueListRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsQueueListRequest(alias)
	return nil
}
