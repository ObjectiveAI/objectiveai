...
type CliCommandAgentsLogsTokenUsageGetRequest struct {
...
func (CliCommandAgentsLogsTokenUsageGetRequest) SchemaTitle() string { return "cli.command.agents.logs.token_usage.get.Request" }
func (v CliCommandAgentsLogsTokenUsageGetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandAgentsLogsTokenUsageGetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"agent_instance_hierarchy", "path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandAgentsLogsTokenUsageGetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandAgentsLogsTokenUsageGetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandAgentsLogsTokenUsageGetRequest(alias)
	return nil
}
