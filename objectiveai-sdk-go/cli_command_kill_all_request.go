...
type CliCommandKillAllRequest struct {
...
func (CliCommandKillAllRequest) SchemaTitle() string { return "cli.command.kill_all.Request" }
func (v CliCommandKillAllRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandKillAllRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandKillAllRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandKillAllRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandKillAllRequest(alias)
	return nil
}
