...
type CliCommandToolsInstallFilesystemRequest struct {
...
func (CliCommandToolsInstallFilesystemRequest) SchemaTitle() string { return "cli.command.tools.install.filesystem.Request" }
func (v CliCommandToolsInstallFilesystemRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandToolsInstallFilesystemRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandToolsInstallFilesystemRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandToolsInstallFilesystemRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandToolsInstallFilesystemRequest(alias)
	return nil
}
