...
type CliCommandDaemonSpawnRequest struct {
	DangerousAdvanced *CliCommandDaemonSpawnRequestDangerousAdvanced `json:"dangerous_advanced,omitempty"`
...
func (CliCommandDaemonSpawnRequest) SchemaTitle() string { return "cli.command.daemon.spawn.Request" }
func (v CliCommandDaemonSpawnRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDaemonSpawnRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDaemonSpawnRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDaemonSpawnRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDaemonSpawnRequest(alias)
	return nil
}
