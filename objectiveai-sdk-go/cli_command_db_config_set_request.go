...
type CliCommandDbConfigSetRequest struct {
	...
	PathType CliCommandDbConfigSetPath `json:"path_type"`
	...
	Value CliCommandDbConfigSetValue `json:"value"`
}

func (CliCommandDbConfigSetRequest) SchemaTitle() string { return "cli.command.db.config.set.Request" }
func (v CliCommandDbConfigSetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDbConfigSetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type", "value"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDbConfigSetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDbConfigSetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDbConfigSetRequest(alias)
	return nil
}
