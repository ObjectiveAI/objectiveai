...
type CliCommandDaemonConfigGetRequest struct {
	...
	PathType CliCommandDaemonConfigGetPath `json:"path_type"`
...
func (CliCommandDaemonConfigGetRequest) SchemaTitle() string { return "cli.command.daemon.config.get.Request" }
func (v CliCommandDaemonConfigGetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDaemonConfigGetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDaemonConfigGetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDaemonConfigGetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDaemonConfigGetRequest(alias)
	return nil
}
