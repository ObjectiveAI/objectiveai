...
type CliError struct {
	...
	Level *CliLevel `json:"level,omitempty"`
	Message JsonValue `json:"message"`
	Type CliErrorType `json:"type"`
}

func (CliError) SchemaTitle() string { return "cli.Error" }
func (v CliError) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliError) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"message", "type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliError: missing required field %q", key)
		}
	}
	type Alias CliError
	var alias Alias
