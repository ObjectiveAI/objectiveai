...
type CliCommandToolsInstallGithubRequest struct {
...
func (CliCommandToolsInstallGithubRequest) SchemaTitle() string { return "cli.command.tools.install.github.Request" }
func (v CliCommandToolsInstallGithubRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandToolsInstallGithubRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"allow_untrusted", "owner", "path_type", "repository"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandToolsInstallGithubRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandToolsInstallGithubRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandToolsInstallGithubRequest(alias)
	return nil
}
