...
type CliCommandDevelopmentViewerGetRequest struct {
	...
	PathType CliCommandDevelopmentViewerGetPath `json:"path_type"`
...
func (CliCommandDevelopmentViewerGetRequest) SchemaTitle() string { return "cli.command.development.viewer.get.Request" }
func (v CliCommandDevelopmentViewerGetRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandDevelopmentViewerGetRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandDevelopmentViewerGetRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandDevelopmentViewerGetRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandDevelopmentViewerGetRequest(alias)
	return nil
}
