...
type CliCommandLaboratoriesSpawnRequest struct {
	...
	PathType CliCommandLaboratoriesSpawnPath `json:"path_type"`
...
func (CliCommandLaboratoriesSpawnRequest) SchemaTitle() string { return "cli.command.laboratories.spawn.Request" }
func (v CliCommandLaboratoriesSpawnRequest) Validate() error {
	return variantValidator.Struct(v)
}

func (v *CliCommandLaboratoriesSpawnRequest) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{"path_type"} {
		if _, ok := raw[key]; !ok {
			return fmt.Errorf("CliCommandLaboratoriesSpawnRequest: missing required field %q", key)
		}
	}
	type Alias CliCommandLaboratoriesSpawnRequest
	var alias Alias
	if err := json.Unmarshal(data, &alias); err != nil {
		return err
	}
	*v = CliCommandLaboratoriesSpawnRequest(alias)
	return nil
}
