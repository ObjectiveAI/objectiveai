...
func TestAgentCompletionChunkPush(t *testing.T) {
	for stream := 0; stream < 20; stream++ {
		t.Run(fmt.Sprintf("stream_%d", stream), func(t *testing.T) {
			...
			for j := 0; j < 20; j++ {
				chunk, err := GenerateAgentCompletionChunk(true, seed)
				if err != nil {
				...
				cffiMerged, err := AgentCompletionChunkMerged(cffiAcc, *chunk)
				if err != nil {
