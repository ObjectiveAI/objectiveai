// THIS FILE IS AUTO-GENERATED. DO NOT EDIT.
// Generated by scripts/install_go.go from objectiveai-json-schema/.
package objectiveai

import (
	"encoding/json"
	"fmt"
)

type CliCommandFunctionsExecuteProfileSpecFavorite string

func (CliCommandFunctionsExecuteProfileSpecFavorite) SchemaVariantTitle() string { return "Favorite" }

type CliCommandFunctionsExecuteProfileSpecFile string

func (CliCommandFunctionsExecuteProfileSpecFile) SchemaVariantTitle() string { return "File" }

type CliCommandFunctionsExecuteProfileSpecPythonInline string

func (CliCommandFunctionsExecuteProfileSpecPythonInline) SchemaVariantTitle() string { return "PythonInline" }

type CliCommandFunctionsExecuteProfileSpecPythonFile string

func (CliCommandFunctionsExecuteProfileSpecPythonFile) SchemaVariantTitle() string { return "PythonFile" }

// CLI-surface form for the `--profile*` argument family — same
// shape as [`FunctionSpec`] but wrapping the profile inline-or-remote
// enum. See [`FunctionSpec`] for the variant semantics.
type CliCommandFunctionsExecuteProfileSpec struct {
	Resolved *FunctionsInlineProfileOrRemoteCommitOptional 
	Favorite *CliCommandFunctionsExecuteProfileSpecFavorite 
	File *CliCommandFunctionsExecuteProfileSpecFile 
	PythonInline *CliCommandFunctionsExecuteProfileSpecPythonInline 
	PythonFile *CliCommandFunctionsExecuteProfileSpecPythonFile 
}

func (v CliCommandFunctionsExecuteProfileSpec) MarshalJSON() ([]byte, error) {
	if v.Resolved != nil {
		return json.Marshal(v.Resolved)
	}
	if v.Favorite != nil {
		return json.Marshal(v.Favorite)
	}
	if v.File != nil {
		return json.Marshal(v.File)
	}
	if v.PythonInline != nil {
		return json.Marshal(v.PythonInline)
	}
	if v.PythonFile != nil {
		return json.Marshal(v.PythonFile)
	}
	return []byte("null"), nil
}

func (v *CliCommandFunctionsExecuteProfileSpec) UnmarshalJSON(data []byte) error {
	{
		var try FunctionsInlineProfileOrRemoteCommitOptional
		if err := json.Unmarshal(data, &try); err == nil {
			candidate := CliCommandFunctionsExecuteProfileSpec{}
			candidate.Resolved = &try
			if candidate.Validate() == nil {
				*v = candidate
				return nil
			}
		}
	}
	{
		var try CliCommandFunctionsExecuteProfileSpecFavorite
		if err := json.Unmarshal(data, &try); err == nil {
			candidate := CliCommandFunctionsExecuteProfileSpec{}
			candidate.Favorite = &try
			if candidate.Validate() == nil {
				*v = candidate
				return nil
			}
		}
	}
	{
		var try CliCommandFunctionsExecuteProfileSpecFile
		if err := json.Unmarshal(data, &try); err == nil {
			candidate := CliCommandFunctionsExecuteProfileSpec{}
			candidate.File = &try
			if candidate.Validate() == nil {
				*v = candidate
				return nil
			}
		}
	}
	{
		var try CliCommandFunctionsExecuteProfileSpecPythonInline
		if err := json.Unmarshal(data, &try); err == nil {
			candidate := CliCommandFunctionsExecuteProfileSpec{}
			candidate.PythonInline = &try
			if candidate.Validate() == nil {
				*v = candidate
				return nil
			}
		}
	}
	{
		var try CliCommandFunctionsExecuteProfileSpecPythonFile
		if err := json.Unmarshal(data, &try); err == nil {
			candidate := CliCommandFunctionsExecuteProfileSpec{}
			candidate.PythonFile = &try
			if candidate.Validate() == nil {
				*v = candidate
				return nil
			}
		}
	}
	return fmt.Errorf("data did not match any variant of CliCommandFunctionsExecuteProfileSpec")
}

func (v CliCommandFunctionsExecuteProfileSpec) Validate() error {
	count := 0
	if v.Resolved != nil { count++ }
	if v.Favorite != nil { count++ }
	if v.File != nil { count++ }
	if v.PythonInline != nil { count++ }
	if v.PythonFile != nil { count++ }
	if count != 1 {
		return fmt.Errorf("CliCommandFunctionsExecuteProfileSpec: exactly one variant must be set, got %d", count)
	}
	return variantValidator.Struct(v)
}
func (CliCommandFunctionsExecuteProfileSpec) SchemaTitle() string { return "cli.command.functions.execute.ProfileSpec" }

